...
Sub GameType.CreateRope( ByVal _linear1    As LinearStateType Ptr, _
	...
	'If ( Not _linear1 = 0 And Not _linear2 = 0 ) Then 
		...
		L.Linear.a = _linear1
		L.Linear.b = LinearStates[ first_linear ]
		
		L.rest_distance = (L.Linear.b->position - L.Linear.a->position).length()
		Dim As Single inverseMass = L.Linear.a->inverse_mass + L.Linear.b->inverse_mass
		L.Linear.reduced_mass = IIf( inverseMass = 0.0 , 0.0 , 1.0 / inverseMass )
		...
		For i As Integer = 0 To num_constraints - 4
			...
			L.Linear.a = LinearStates[ first_linear + i ]
			L.Linear.b = LinearStates[ first_linear + i + 1 ]
		...
		L.Linear.a = LinearStates.P_back
		L.Linear.b = _linear2
		
		L.rest_distance = (L.Linear.b->position - L.Linear.a->position).length()
		inverseMass = L.Linear.a->inverse_mass + L.Linear.b->inverse_mass
		L.Linear.reduced_mass = IIf( inverseMass = 0.0 , 0.0 , 1.0 / inverseMass )
...
Function GameType.CreatePlanet( ByVal _position As Vec2, _
	...
	Planet.Angular.direction_Vector = Vec2( Cos( _angle ) , Sin( _angle ) )
...
Sub GameType.CreateGirder( ByVal _position As Vec2, _
	...
	for i as integer = 0 to _width - 1
		For j as integer = 0 to _length - 2
			...
			S.Linear.a = LinearStates[ first_linear + i          + j * _width ]
			S.Linear.b = LinearStates[ first_linear + i + _width + j * _width ]
	...
	for i as integer = 0 to _length - 1
		For j as integer = 0 to _width - 2
			...
			S.Linear.a = LinearStates[ first_linear + i * _width + j     ]
			S.Linear.b = LinearStates[ first_linear + i * _width + j + 1 ]
	...
	If ( _type = 0 Or _type = 3 ) Then
		...
		for i as integer = 0 to _width - 2
			For j as integer = 0 to _length - 2' Step 2
				...
				S.Linear.a = LinearStates[ first_linear +   i + j   * _width         ]
				S.Linear.b = LinearStates[ first_linear + ( j + 1 ) * _width + i + 1 ]
	...
	If ( _type = 1 Or _type = 3 ) Then
		...
		For i as integer = 0 to _width - 2
			For j as integer = 0 to _length - 2' Step 2
				...
				S.Linear.a = LinearStates[ first_linear + ( j + 1 ) *_width + i     ]
				S.Linear.b = LinearStates[ first_linear +   j       *_width + i + 1 ]
