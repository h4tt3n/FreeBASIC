...
Sub GameType.ComputeData()
	...
	If ( Not Gravitys.Empty ) Then 
		...
		For G As GravityType Ptr = Gravitys.P_front To Gravitys.P_back
			...
			Dim As Vec2 distance_vector = G->Linear.b->position - G->Linear.a->position
			
			G->Linear.unit_vector = distance_vector.Unit()
			
			Dim As Single distance_squared = distance_vector.lengthsquared()
			
			Dim As Single gravity_impulse = -( cgravity * INV_DT * G->Linear.a->mass * G->Linear.b->mass ) / distance_squared
			
			G->Linear.a->impulse -= gravity_impulse * G->Linear.a->inverse_mass * G->Linear.unit_vector
			G->Linear.b->impulse += gravity_impulse * G->Linear.b->inverse_mass * G->Linear.unit_vector
	...
	If ( Not LL_constraints.Empty ) Then 
		...
		For S As LL_constraintType Ptr = LL_constraints.P_front To LL_constraints.P_back
			...
			Dim As Vec2 distance = S->Linear.b->position - S->Linear.a->position
			Dim As Vec2 velocity = S->Linear.b->velocity - S->Linear.a->velocity
			
			S->Linear.unit_vector = distance.Unit()
			
			Dim As Single distance_error = S->Linear.unit_vector.dot( distance ) - S->rest_distance
			Dim As Single velocity_error = S->Linear.unit_vector.dot( velocity )
			
			S->rest_impulse = - cstiffness * distance_error * INV_DT - cdamping * velocity_error
			
			'' spring state vectors
			If ( S->Linear.a->inverse_mass = 0.0 ) Then
				
				S->Linear.position = S->Linear.a->position
				S->Linear.velocity = Vec2( 0.0, 0.0 )
				
			ElseIf ( S->Linear.b->inverse_mass = 0.0 ) Then
				
				S->Linear.position = S->Linear.b->position
				S->Linear.velocity = Vec2( 0.0, 0.0 )
				
			Else
				
				S->Linear.position = ( S->Linear.a->position * S->Linear.a->mass + _
			                          S->Linear.b->position * S->Linear.b->mass ) / _
			                        ( S->Linear.a->mass + S->Linear.b->mass )
				
				S->Linear.velocity = ( S->Linear.a->velocity * S->Linear.a->mass + _
			                          S->Linear.b->velocity * S->Linear.b->mass ) / _
			                        ( S->Linear.a->mass + S->Linear.b->mass )
				
			End If
			
			'' particle local state vectors relative to spring state vector
			Dim As Vec2 local_position_a = S->Linear.a->Position - S->Linear.Position
			Dim As Vec2 local_position_b = S->Linear.b->Position - S->Linear.Position
			
			Dim As Vec2 local_velocity_a = S->Linear.a->Velocity - S->Linear.Velocity
			Dim As Vec2 local_velocity_b = S->Linear.b->Velocity - S->Linear.Velocity
			
			'' moment of inertia and inverse
			S->Angular.inertia = local_position_a.lengthsquared() * S->Linear.a->mass + _
			                     local_position_b.lengthsquared() * S->Linear.b->mass
			
			S->Angular.inverse_inertia = IIf( S->Angular.inertia > 0.0 , 1.0 / S->Angular.inertia, 0.0 )
			
			'' angular velocity
			S->Angular.velocity = ( local_position_a.PerpDot( local_velocity_a * S->Linear.a->mass ) + _
		                           local_position_b.PerpDot( local_velocity_b * S->Linear.b->Mass ) ) * _
		                           S->Angular.inverse_inertia
	...
	If ( Not AngularConstraints.Empty ) Then 
		...
		For A As AngularConstraintType Ptr = AngularConstraints.P_front To AngularConstraints.P_back
			...
			A->Angular.delta_direction_vector = Vec2( A->Angular.a->direction_vector.dot( A->Angular.b->direction_vector ), _
			                                          A->Angular.a->direction_vector.perpdot( A->Angular.b->direction_vector ) )
			
			'' errors
			'Dim As Single direction_error = ATan2( A->direction.perpdot( A->rest_direction ), _
			'                                       A->direction.dot( A->rest_direction ) )
			
			Dim As Single direction_error = A->rest_delta_direction.perpdot( A->Angular.delta_direction_vector )
			
			Dim As Single velocity_error = A->Angular.b->velocity - A->Angular.a->velocity
			
			'' reduced moment of inertia
			Dim As Single inverse_inertia = A->Angular.a->inverse_inertia + A->Angular.b->inverse_inertia
			
			A->Angular.reduced_inertia = IIf( inverse_inertia > 0.0 , 1.0 / inverse_inertia , 0.0 )
...
Sub GameType.ApplyCorrectiveimpulse()
	...
	If ( Not LL_constraints.Empty ) Then 
		...
		For S As LL_constraintType Ptr = LL_constraints.P_front To LL_constraints.P_back
			...
			''	delta impulse
			Dim As Vec2 delta_impulse = S->Linear.b->impulse - S->Linear.a->impulse
			
			'' impulse error
			Dim As Single impulse_error = S->Linear.unit_vector.dot( delta_impulse ) - S->rest_impulse
			
			'' corrective impulse
		   Dim As Vec2 corrective_impulse = -impulse_error * S->Linear.reduced_mass * S->Linear.unit_vector
			
			S->Linear.a->impulse -= corrective_impulse * S->Linear.a->inverse_mass
			S->Linear.b->impulse += corrective_impulse * S->Linear.b->inverse_mass
...
Sub GameType.ApplyWarmStart()
	...
	If ( Not LL_constraints.Empty ) Then 
		...
		For S As LL_constraintType Ptr = LL_constraints.P_front To LL_constraints.P_back
			...
			Dim As Single projected_impulse = S->Linear.unit_vector.dot( S->accumulated_impulse )
			
			If ( projected_impulse < 0.0 ) Then
				
				Dim As Vec2 warmstart_impulse = cwarmstart * projected_impulse * S->Linear.unit_vector
				
				S->Linear.a->impulse -= warmstart_impulse * S->Linear.a->inverse_mass 
				S->Linear.b->impulse += warmstart_impulse * S->Linear.b->inverse_mass
...
Sub GameType.UpdateScreen()
	...
	If ( Not Gravitys.Empty ) Then 
		...
		For G As GravityType Ptr = Gravitys.P_front To Gravitys.P_back
			...
			Line(G->Linear.a->position.x, G->Linear.a->position.y)-_
				 (G->Linear.b->position.x, G->Linear.b->position.y), RGB(255, 0, 255)
	...
	If ( Not LL_constraints.Empty ) Then 
		...
		For S As LL_constraintType Ptr = LL_constraints.P_front To LL_constraints.P_back Step 1
			...
			Line(S->Linear.a->position.x, S->Linear.a->position.y)-_
				 (S->Linear.b->position.x, S->Linear.b->position.y), RGB(192, 192, 192)
...
Sub GameType.CreateScreen( ByVal Wid As Integer, ByVal Hgt As Integer )
   ...
   WindowTitle "Graveyard Orbit - a 2D space physics puzzle game by Michael Schmidt Nissen. Prototype # 2, december 2016"
