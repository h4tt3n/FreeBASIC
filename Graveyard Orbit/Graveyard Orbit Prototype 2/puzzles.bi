...
Sub GameType.Puzzle1()
	...
	For i As Integer = 0 To num_Springs-1
		...
		S.Linear.a    = LinearStates[i]
		S.Linear.b    = LinearStates[i+1]
		S.rest_distance = (S.Linear.b->position - S.Linear.a->position).length()
		
		Dim As Single inverseMass = S.Linear.a->inverse_mass + S.Linear.b->inverse_mass
		
		S.Linear.reduced_mass = IIf( inverseMass = 0.0 , 0.0 , 1.0 / inverseMass )
...
Sub GameType.Puzzle2()
	...
	For i As Integer = 0 To num_Springs - 1
		...
		S.Linear.b    = LinearStates[i+1]
		S.rest_distance = ( S.Linear.b->position - S.Linear.a->position ).length()
		S.Linear.unit_vector = ( S.Linear.b->position - S.Linear.a->position ).unit()
		
		Dim As Single inverseMass = S.Linear.a->inverse_mass + S.Linear.b->inverse_mass
		
		S.Linear.reduced_mass = IIf( inverseMass > 0.0 , 1.0 / inverseMass , 0.0 )
	...
	For i As Integer = 0 To num_angular_Springs-1 
		...
		A.Angular.a = @LL_constraints[ i ].Angular
		A.Angular.b = @LL_constraints[i + 1].Angular
...
Sub GameType.Puzzle4()
	...
	G1.Linear.a = @PP1->Linear
	G1.Linear.b = @PP2->Linear
	
	Gravitys.push_back( G1 )
	
	Dim as GravityType G2
	
	G2.Linear.a = @PP3->Linear
	G2.Linear.b = @PP2->Linear
	
	Gravitys.push_back( G2 )
	
	Dim as GravityType G3
	
	G3.Linear.a = @PP1->Linear
	G3.Linear.b = @PP3->Linear
	
	Gravitys.push_back( G3 )
	
	'Dim As LL_ConstraintType Ptr S = CreateSpring()
	'
	'S->Linear.a = @Particle(1)
	'S->Linear.b = @Particle(2)
	'
End Sub
...
Sub GameType.Puzzle5()
	...
	R.Angular.direction_Vector = Vec2( Cos( 0.0 ) , Sin( 0.0 ) )
	...
	S.Linear.a = @RigidBodys[0]->Roxels[0]->linear
	S.Linear.b = @Roxels[0]->Linear
	...
	''
	S.Linear.a = @RigidBodys[0]->Roxels[1]->linear
	S.Linear.b = @Roxels[1]->Linear
	
	S.rest_distance = (S.Linear.b->position - S.Linear.a->position).length()
	
	inverseMass = S.Linear.a->inverse_mass + S.Linear.b->inverse_mass
...
Sub GameType.Puzzle6()
	...
	''
	RigidBodys[0]->Angular.impulse =  -80.0
	RigidBodys[1]->Angular.impulse =  80.0
	RigidBodys[2]->Angular.impulse =  -80.0
	RigidBodys[3]->Angular.impulse =  80.0
	...
	S.Linear.a = @RigidBodys[0]->Roxels[0]->Linear
	S.Linear.b = @RigidBodys[1]->Linear
	...
	''
	S.Linear.a = @RigidBodys[1]->Roxels[1]->Linear
	S.Linear.b = @RigidBodys[2]->Linear
	...
	''
	S.Linear.a = @RigidBodys[2]->Roxels[2]->Linear
	S.Linear.b = @RigidBodys[3]->Linear
	...
	''
	S.Linear.a = @RigidBodys[3]->Roxels[3]->Linear
	S.Linear.b = @RigidBodys[0]->Linear
