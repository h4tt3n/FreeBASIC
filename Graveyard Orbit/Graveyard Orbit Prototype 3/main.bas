...
Sub RigidBodyType.computeData()
	...
	If ( Not Roxels.Empty ) Then
		...
		LinearState.inverse_mass = IIf( LinearState.Mass > 0.0 , 1.0 / LinearState.Mass , 0.0 )
		...
		LinearState.Position *= LinearState.inverse_mass
		LinearState.Velocity *= LinearState.inverse_mass
		
		'' compute moment of inertial and inverse
		AngularState.inertia = 0.0
		AngularState.inverse_inertia = 0.0
		...
		AngularState.inverse_inertia = IIf( AngularState.inertia > 0.0 , 1.0 / AngularState.inertia , 0.0 )
		...
		AngularState.velocity *= AngularState.inverse_inertia
...
Sub SoftBodyType.computeData()
	...
	If ( Not LinearStates.Empty ) Then
		...
		For R As LinearStateType Ptr = LinearStates.p_front To LinearStates.p_back
			
			LinearState.Mass += R->mass
		...
		For R As LinearStateType Ptr = LinearStates.p_front To LinearStates.p_back
			...
			LinearState.Position += R->position * R->mass
			LinearState.Velocity += R->velocity * R->mass
		...
		For R As LinearStateType Ptr = LinearStates.p_front To LinearStates.p_back
			...
			Dim As Vec2 local_position = R->Position - LinearState.Position
			
			AngularState.inertia += local_position.LengthSquared() * R->mass 
		...
		For R As LinearStateType Ptr = LinearStates.p_front To LinearStates.p_back
			...
			Dim As Vec2 local_position = R->Position - LinearState.Position
			Dim As Vec2 local_velocity = R->Velocity - LinearState.Velocity
			
			AngularState.velocity += local_position.Perpdot( local_velocity * R->mass )
...
Sub GameType.DistributeImpulses()
	...
	'		
	'		Dim As Vec2 local_linear = S->LinearLink.a->impulse * S->LinearLink.a->mass +_
	'		                           S->LinearLink.b->impulse * S->LinearLink.b->mass
	'		
	'		S->LinearLink.AddImpulse( local_linear * S->LinearLink.inverse_mass - S->LinearLink.linear_prev )
	'		
	'		S->LinearLink.a->impulse = S->LinearLink.impulse
	'		S->LinearLink.b->impulse = S->LinearLink.impulse
	'		
	'		S->LinearLink.linear_prev = S->LinearLink.impulse
	'		
	'	Next
	'	
	'EndIf
	...
	If ( Not Rigidbodys.empty ) Then
		...
		For R As RigidBodyType Ptr = rigidbodys.p_front To rigidbodys.p_back
			...
			If ( Not R->Roxels.Empty ) Then 
				...
				R->LinearState.AddImpulse( local_linear  * R->LinearState.Inverse_mass     - R->Linear_prev )
				
				'R->LinearState.impulse  += local_linear  * R->LinearState.Inverse_mass     - R->Linear_prev
				R->AngularState.impulse += local_angular * R->AngularState.inverse_inertia - R->Angular_prev
				...
				For P As RoxelType Ptr = R->Roxels.p_front To R->Roxels.p_back
					...
					P->LinearState.impulse = R->LinearState.impulse + local_position.perpdot( R->AngularState.impulse )
					
				Next
				
				'' Save global for next iteration
				R->Linear_prev  = R->LinearState.impulse
				R->Angular_prev = R->AngularState.impulse
...
Sub RocketType.ApplyThrustImpulse( ByVal _throttle As Single )
	...
	Dim As Single thrust_impulse = burned_fuel * ExhaustVelocity_ * LinearState.inverse_mass
	
	LinearState.Impulse += thrust_impulse * Angular.direction_vector * Sgn( _throttle )
	
	FuelMass_ -= burned_fuel
	
	LinearState.Mass = DryMass_ + FuelMass_
...
Sub GameType.ComputeData()
	...
	If ( Not Gravitys.Empty ) Then 
		...
		For G As GravityType Ptr = Gravitys.P_front To Gravitys.P_back
			...
			Dim As Vec2 distance_vector = G->LinearLink.b->position - G->LinearLink.a->position
			
			G->LinearLink.unit_vector = distance_vector.Unit()
			
			Dim As Single distance_squared = distance_vector.lengthsquared()
			
			Dim As Single gravity_impulse = -( cgravity * INV_DT * G->LinearLink.a->mass * G->LinearLink.b->mass ) / distance_squared
			
			G->LinearLink.a->Addimpulse( -gravity_impulse * G->LinearLink.a->inverse_mass * G->LinearLink.unit_vector )
			G->LinearLink.b->Addimpulse(  gravity_impulse * G->LinearLink.a->inverse_mass * G->LinearLink.unit_vector )
	...
	If ( Not LinearSprings.Empty ) Then 
		...
		For S As LinearSpringType Ptr = LinearSprings.P_front To LinearSprings.P_back
			...
			Dim As Vec2 distance = S->LinearLink.b->position - S->LinearLink.a->position
			Dim As Vec2 velocity = S->LinearLink.b->velocity - S->LinearLink.a->velocity
			
			S->LinearLink.unit_vector = distance.Unit()
			
			Dim As Single distance_error = S->LinearLink.unit_vector.dot( distance ) - S->rest_distance
			Dim As Single velocity_error = S->LinearLink.unit_vector.dot( velocity )
			
			S->rest_impulse = - cstiffness * distance_error * INV_DT - cdamping * velocity_error
			
			'' spring state vectors
			If ( S->LinearLink.a->inverse_mass = 0.0 ) Then
				
				S->LinearLink.position = S->LinearLink.a->position
				S->LinearLink.velocity = Vec2( 0.0, 0.0 )
				
			ElseIf ( S->LinearLink.b->inverse_mass = 0.0 ) Then
				
				S->LinearLink.position = S->LinearLink.b->position
				S->LinearLink.velocity = Vec2( 0.0, 0.0 )
				
			Else
				
				S->LinearLink.position = ( S->LinearLink.a->position * S->LinearLink.a->mass + _
			                              S->LinearLink.b->position * S->LinearLink.b->mass ) / _
			                              ( S->LinearLink.a->mass + S->LinearLink.b->mass )
				
				S->LinearLink.velocity = ( S->LinearLink.a->velocity * S->LinearLink.a->mass + _
			                              S->LinearLink.b->velocity * S->LinearLink.b->mass ) / _
			                              ( S->LinearLink.a->mass + S->LinearLink.b->mass )
				
			End If
			
			'' particle local state vectors relative to spring state vector
			Dim As Vec2 local_position_a = S->LinearLink.a->Position - S->LinearLink.Position
			Dim As Vec2 local_position_b = S->LinearLink.b->Position - S->LinearLink.Position
			
			Dim As Vec2 local_velocity_a = S->LinearLink.a->Velocity - S->LinearLink.Velocity
			Dim As Vec2 local_velocity_b = S->LinearLink.b->Velocity - S->LinearLink.Velocity
			
			'' moment of inertia and inverse
			S->AngularState.inertia = local_position_a.lengthsquared() * S->LinearLink.a->mass + _
			                          local_position_b.lengthsquared() * S->LinearLink.b->mass
			
			S->AngularState.inverse_inertia = IIf( S->AngularState.inertia > 0.0 , 1.0 / S->AngularState.inertia, 0.0 )
			
			'' angular velocity
			S->AngularState.velocity = ( local_position_a.PerpDot( local_velocity_a * S->LinearLink.a->mass ) + _
		                                local_position_b.PerpDot( local_velocity_b * S->LinearLink.b->Mass ) ) * _
		                                S->AngularState.inverse_inertia
	...
	If ( Not AngularConstraints.Empty ) Then 
		...
		For A As AngularConstraintType Ptr = AngularConstraints.P_front To AngularConstraints.P_back
			...
			A->AngularLink.delta_direction_vector = Vec2( A->AngularLink.a->direction_vector.dot( A->AngularLink.b->direction_vector ), _
			                                              A->AngularLink.a->direction_vector.perpdot( A->AngularLink.b->direction_vector ) )
			
			'' errors
			'Dim As Single direction_error = ATan2( A->direction.perpdot( A->rest_direction ), _
			'                                       A->direction.dot( A->rest_direction ) )
			
			Dim As Single direction_error = A->rest_delta_direction.perpdot( A->AngularLink.delta_direction_vector )
			
			Dim As Single velocity_error = A->AngularLink.b->velocity - A->AngularLink.a->velocity
			
			'' reduced moment of inertia
			Dim As Single inverse_inertia = A->AngularLink.a->inverse_inertia + A->AngularLink.b->inverse_inertia
			
			A->AngularLink.reduced_inertia = IIf( inverse_inertia > 0.0 , 1.0 / inverse_inertia , 0.0 )
			
			'' desired "rest impulse" needed to satisfy the constraint
			A->rest_impulse = - castiffness * direction_error * INV_DT - cadamping * velocity_error 
			'A->rest_impulse = - cadamping * velocity_error 
...
Sub GameType.ApplyCorrectiveimpulse()
	...
	If ( Not AngularConstraints.Empty ) Then 
		...
		For A As AngularConstraintType Ptr = AngularConstraints.P_front To AngularConstraints.P_back
			...
			''' 
			'Dim As LinearSpringType Ptr S_a = A->spring_a
			'Dim As LinearSpringType Ptr S_b = A->spring_b
			'
			''' current impulse
			'Dim As Single AngularImpulse_a = S_a->Local_Position_a.PerpDot( S_a->Linear.a->Impulse ) * S_a->inverse_Inertia * S_a->Linear.a->Mass +_
			'                                 S_a->Local_Position_b.PerpDot( S_a->Linear.b->Impulse ) * S_a->inverse_Inertia * S_a->Linear.b->Mass
			'
			'Dim As Single AngularImpulse_b = S_b->Local_position_a.PerpDot( S_b->Linear.a->Impulse ) * S_b->inverse_Inertia * S_b->Linear.a->Mass + _
			'                                 S_b->Local_position_b.PerpDot( S_b->Linear.b->Impulse ) * S_b->inverse_Inertia * S_b->Linear.b->Mass
			'
			'                                 
			''' corrective impulse
			...
			'
			'S_a->Linear.a->Impulse += S_a->Local_Position_a.PerpDot( new_Angular_Impulse_a )
			'S_a->Linear.b->Impulse += S_a->Local_Position_b.PerpDot( new_Angular_Impulse_a )
		
			'S_b->Linear.a->Impulse += S_b->Local_Position_a.PerpDot( new_Angular_Impulse_b )
			'S_b->Linear.b->Impulse += S_b->Local_Position_b.PerpDot( new_Angular_Impulse_b )
	...
	If ( Not LinearSprings.Empty ) Then 
		...
		For S As LinearSpringType Ptr = LinearSprings.P_front To LinearSprings.P_back
			...
			''	delta impulse
			Dim As Vec2 delta_impulse = S->LinearLink.b->impulse - S->LinearLink.a->impulse
			
			'' impulse error
			Dim As Single impulse_error = S->LinearLink.unit_vector.dot( delta_impulse ) - S->rest_impulse
			
			'' corrective impulse
		   Dim As Vec2 corrective_impulse = -impulse_error * S->LinearLink.reduced_mass * S->LinearLink.unit_vector
			
			S->LinearLink.a->AddImpulse( -corrective_impulse * S->LinearLink.a->inverse_mass )
			S->LinearLink.b->AddImpulse(  corrective_impulse * S->LinearLink.b->inverse_mass )
			
			'S->LinearLink.a->impulse -= corrective_impulse * S->LinearLink.a->inverse_mass
			'S->LinearLink.b->impulse += corrective_impulse * S->LinearLink.b->inverse_mass
...
Sub GameType.ApplyWarmStart()
	...
	'		
	'		'' apply linear
	'		A->spring_a->Linear.a->Impulse -= S_a->Local_Position_a.PerpDot( new_Angular_Impulse_a )
	'		A->spring_a->Linear.b->Impulse -= S_a->Local_Position_b.PerpDot( new_Angular_Impulse_a )
	'		
	'		A->spring_b->Linear.a->Impulse += S_b->Local_Position_a.PerpDot( new_Angular_Impulse_b )
	'		A->spring_b->Linear.b->Impulse += S_b->Local_Position_b.PerpDot( new_Angular_Impulse_b )
	'		
	'		''
	...
	If ( Not LinearSprings.Empty ) Then 
		...
		For S As LinearSpringType Ptr = LinearSprings.P_front To LinearSprings.P_back
			...
			Dim As Single projected_impulse = S->LinearLink.unit_vector.dot( S->accumulated_impulse )
			
			If ( projected_impulse < 0.0 ) Then
				
				Dim As Vec2 warmstart_impulse = cwarmstart * projected_impulse * S->LinearLink.unit_vector
				
				S->LinearLink.a->AddImpulse( -warmstart_impulse * S->LinearLink.a->inverse_mass )
				S->LinearLink.b->AddImpulse(  warmstart_impulse * S->LinearLink.b->inverse_mass )
				
				'S->LinearLink.a->impulse -= warmstart_impulse * S->LinearLink.a->inverse_mass 
				'S->LinearLink.b->impulse += warmstart_impulse * S->LinearLink.b->inverse_mass
...
Sub GameType.ComputeNewState()
	...
	If ( Not Roxels.Empty ) Then 
		...
		For P As RoxelType Ptr = Roxels.p_front To Roxels.p_back
			...
			If ( P->LinearState.inverse_mass > 0.0 And P <> picked ) Then
				
				P->LinearState.velocity += P->LinearState.impulse
				P->LinearState.position += P->LinearState.velocity * DT 
	...
	If ( Not Roxels.Empty ) Then 
		...
		For P As RoxelType Ptr = Roxels.p_front To Roxels.p_back
			...
			If ( P->radius > 0.0 ) Then
				...
				Dim As UInteger Col = IIf( P->LinearState.inverse_mass = 0.0, RGB(160, 160, 160), RGB(255, 0, 0) )
				
				Circle(P->LinearState.position.x, P->LinearState.position.y), P->radius, Col,,, 1, f
				
			Else
				
				Dim As UInteger Col = IIf( P->LinearState.inverse_mass = 0.0, RGB(160, 160, 160), RGB(160, 160, 160) )
...
Sub GameType.CreateScreen( ByVal Wid As Integer, ByVal Hgt As Integer )
   ...
   WindowTitle "Graveyard Orbit - a 2D space physics puzzle game. Prototype # 3, april 2017"
...
Sub GameType.UpdateInput()
	...
	If ( ScreenEvent( @e ) ) Then
		...
		Case fb.EVENT_KEY_PRESS
			...
			If ( e.scancode = fb.SC_F8 ) Then Puzzle8()
