...
Function GameType.CreateLinearState( ByVal _position As Vec2, _
	...
	L.mass     = _mass
	L.inv_mass = IIf( L.mass <> 0.0 , 1.0 / L.mass, 0.0 )
...
Function GameType.CreateRoxel( ByVal _position As Vec2, _
	...
	R.LinearState.position = _position
	R.LinearState.mass     = _mass
	R.LinearState.inv_mass = IIf( _mass <> 0.0 , 1.0 / _mass, 0.0 )
	
	R.radius = IIf( _radius <> 0.0, _radius , ( ( _mass / C_DENSITY ) / (4/3) * PI ) ^ (1/3) )
...
Function GameType.CreateGravity( ByVal _linear1 As LinearStateType Ptr, _
	...
	GP->LinearLink.inv_Mass = IIf( GP->LinearLink.mass <> 0.0 , 1.0 / GP->LinearLink.mass, 0.0 )
...
Function GameType.CreateLinearSpring( ByVal _stiffness  As Single, _
	...
	LP->LinearLink.Position = ( LP->LinearLink.a->position * LP->LinearLink.a->mass + _
	                            LP->LinearLink.b->position * LP->LinearLink.b->mass ) * LP->LinearLink.inv_Mass
...
Function GameType.CreatePlanet( ByVal _position As Vec2, _
	...
	For i As Integer = 0 To _roxels - 1
		...
		RP->LinearState.Mass     = 1.0' + Rnd() * 90.0
...
Function GameType.CreateSoftGirder( ByVal _position As Vec2, _
	...
	For y as integer = 0 to _size.y - 1
		For x as integer = 0 to _size.x - 2
			...
			Dim As LinearSpringType F
			
			Dim As LinearSpringType Ptr FP = SP->LinearSprings.push_back( F )
			
			FP->LinearLink.a = SP->LinearStates[ x + y * _size.x     ]
			FP->LinearLink.b = SP->LinearStates[ x + y * _size.x + 1 ]
			
			FP->rest_distance = ( FP->LinearLink.b->position - FP->LinearLink.a->position ).length()
