...
Function GameType.CreateGearJoint( ByVal _Angular1 As AngularStateType Ptr, _
	                                ByVal _Angular2 As AngularStateType Ptr, _
	                                ByVal _Radius1 As Single, _ 
	                                ByVal _Radius2 As Single ) As GearJointType Ptr                            
	
	Dim As GearJointType G
	
	Dim As GearJointType Ptr GP = GearJoints.push_Back( G )
	
	GP->body_a = _Angular1
	GP->body_b = _Angular2
	
	GP->radius_a = _Radius1
	GP->radius_b = _Radius2
	
	GP->c_warmstart = DEFAULT_JOINT_WARMSTART
	
	'' Compute M
	Dim As Single K1 = GP->body_a->inv_mass + GP->body_b->inv_mass
	Dim As Single K2 = GP->body_a->inv_inertia * GP->radius_a * GP->radius_a
	Dim As Single K3 = GP->body_b->inv_inertia * GP->radius_b * GP->radius_b
	
	Dim As Single K = K1 + K2 + K3
	
	GP->M = IIf( K > 0.0 , 1.0 / K , 0.0 )
