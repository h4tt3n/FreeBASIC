...
Type GameType
	...
	Declare Function CreateGearJoint( ByVal _Angular1 As AngularStateType Ptr, _
	                                  ByVal _Angular2 As AngularStateType Ptr, _
	                                  ByVal _Radius1 As Single, _ 
	                                  ByVal _Radius2 As Single  ) As GearJointType Ptr
...
Constructor GameType()
	...
	gearjoints.Reserve( MAX_GEAR_JOINTS )
...
Sub GameType.ComputeRestImpulse()
	...
	'' Gear Joints
	If ( Not GearJoints.Empty ) Then 
		
		For G As GearJointType Ptr = GearJoints.P_front To GearJoints.P_back
...
Sub GameType.ApplyCorrectiveimpulse()
	...
	If ( Not GearJoints.Empty ) Then 
		...
		For G As GearJointType Ptr = GearJoints.P_front To GearJoints.P_back Step 1
			
			G->ApplyCorrectiveImpulse()
			
		Next
		
		For G As GearJointType Ptr = GearJoints.P_back To GearJoints.P_front Step -1
...
Sub GameType.UpdateInput()
	...
	position = Vec2( Cast( Single, mouse_x ) , Cast( Single, ( SCREEN_HEIGHT - mouse_y ) ) )
