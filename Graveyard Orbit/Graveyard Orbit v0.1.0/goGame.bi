...
Type Game
	...
	'                               ByVal _type   As UInteger, _
	'                               ByVal _flags  As UInteger ) As SoftBody Ptr
	
	Declare Function CreateGirder( ByVal _points As Vec2, _
	                               ByVal _Length As Vec2, _
	                               ByVal _type   As integer ) As SoftBody Ptr
	
	Declare Function CreateGirder OverLoad ( ByVal _points  As Vec2, _
	                                         ByVal _Length  As Vec2, _
...
Sub Game.RunGameLoop()
	...
	Do
		...
		''
		If MultiKey( fb.SC_0 ) Then Intro()
		...
		If MultiKey( fb.SC_SPACE ) Then Player->SetVelocity( Vec2( 0.0, 0.0 ) )
...
Sub Game.RenderToScreen()
	...
	'Locate 6,2: Print Rockets_[0]->GetMass
	'Locate 7,2: Print Rockets_[0]->GetInverseMass
	
	'If ( Not GameWorld.SoftBodys_.Empty ) Then 
	'	
	'	Locate 6,2: Print GameWorld.SoftBodys_[0]->GetAngularVelocity
	'	
	'EndIf
	'
	'If ( Not GameWorld.RigidBodys_.Empty ) Then 
	'	
	'	Locate 8,2: Print GameWorld.RigidBodys_[0]->GetAngularVelocity
	'	
	'EndIf
	...
	If ( Not Roxels_.Empty ) Then
		...
		For R As Roxel Ptr = Roxels_.p_front To Roxels_.p_back
			...
			If R->GetFlag( IS_VISIBLE ) Then
				...
				Circle( R->GetPosition.x, R->GetPosition.y ), R->GetRadius + 32.0 , R->GetBackGroundColour,,, 1, f
	...
	If ( Not Planets_.Empty ) Then 
		...
		For PP As Planet Ptr = Planets_.p_front To Planets_.p_back
			...
			If PP->GetFlag( IS_VISIBLE ) Then
				...
				Dim As Vec2 nrm = PP->GetPosition + PP->GetAngleVector         * 16
				Dim As Vec2 lft = PP->GetPosition + PP->GetAngleVector.PerpCCW * 16
				Dim As Vec2 rgt = PP->GetPosition + PP->GetAngleVector.PerpCW  * 16
				
				Line( PP->GetPosition.x, PP->GetPosition.y )-( nrm.x, nrm.y ), RGB( 255, 255, 0 )
				...
				Circle( PP->GetPosition.x, PP->GetPosition.y ), 2.0, RGB( 255, 255, 0 ),,, 1, f
	...
	If ( Not GameWorld.SoftBodys_.Empty ) Then 
		...
		For S As SoftBody Ptr = GameWorld.SoftBodys_.p_front To GameWorld.SoftBodys_.p_back
			...
			If S->GetFlag( IS_VISIBLE ) Then
				...
				Dim As Vec2 nrm = S->GetPosition + S->GetAngleVector         * 512
				Dim As Vec2 lft = S->GetPosition + S->GetAngleVector.PerpCCW * 16
				Dim As Vec2 rgt = S->GetPosition + S->GetAngleVector.PerpCW  * 16
