...
Sub Game.PrepareGame()
	...
	Color( RGB( 192, 192, 192 ), RGB( 12, 16, 24 ) )
	
	WindowTitle "Graveyard Orbit - A 2D space physics puzzle game - v. 0.2.0 - Press 1-9 for demo levels"
...
Sub Game.RunGameLoop()
	...
	Do
		...
		If ( Not GameWorld.RigidBodys_.Empty ) Then
			...
			'If MultiKey( fb.SC_UP )    Then GameWorld.RigidBodys_[0]->AddAngle( 0.1 )
			'If MultiKey( fb.SC_DOWN )  Then GameWorld.RigidBodys_[0]->AddAngle( Vec2(   0.0, -10.0 ) )
			If MultiKey( fb.SC_LEFT )  Then GameWorld.RigidBodys_[0]->AddAngularVelocity(  0.01 )
			If MultiKey( fb.SC_RIGHT ) Then GameWorld.RigidBodys_[0]->AddAngularVelocity( -0.01 )
			
			'If MultiKey( fb.SC_UP )    Then GameWorld.RigidBodys_[0]->AddVelocity( Vec2(   0.0,  10.0 ) )
			'If MultiKey( fb.SC_DOWN )  Then GameWorld.RigidBodys_[0]->AddVelocity( Vec2(   0.0, -10.0 ) )
			'If MultiKey( fb.SC_LEFT )  Then GameWorld.RigidBodys_[0]->AddVelocity( Vec2( -10.0,   0.0 ) )
			'If MultiKey( fb.SC_RIGHT ) Then GameWorld.RigidBodys_[0]->AddVelocity( Vec2(  10.0,   0.0 ) )
		...
		If ( Not Player->Grapple_ = 0 ) Then 
			...
			Player->Grapple_->ComputeMass()
			Player->Grapple_->ComputeStateVectors()
	...
	If ( Not GameWorld.NewtonGravitys_.Empty ) Then 
		...
		For NP As NewtonGravity Ptr = GameWorld.NewtonGravitys_.p_front To GameWorld.NewtonGravitys_.p_back
			...
			If NP->GetFlag( IS_VISIBLE ) Then
				...
				Line( NP->GetLinearStateB->GetPosition.x, NP->GetLinearStateB->GetPosition.y )-_
				    ( NP->GetLinearStateA->GetPosition.x, NP->GetLinearStateA->GetPosition.y ), RGB( 64, 32, 64 )
				
				Circle( NP->GetPosition.x, NP->GetPosition.y ), 4.0, RGB( 96, 48, 96 ),,,1,f
