...
Sub Game.Puzzle1()
	...
	World_description_ = "World 1: Single planet"
	
	Puzzle_description_ = "Puzzle: "
	
	Dim As ShapeBody Ptr G1 = CreateArchedShapeGirder( Vec2( 24, 2 ), Vec2( 256.0, 256.0 ), 0.2 * TWO_PI, 8000, X_TRUSS )
	'Dim As SoftBody Ptr G1 = CreateArchedGirder( Vec2( 24, 2 ), Vec2( 256.0, 256.0 ), 0.2 * TWO_PI, 8000, X_TRUSS )
	
	G1->SetPosition( Vec2( 10000.0, 10000.0 ) )
	
	Dim As RigidBody Ptr Asteroid1 = CreateAsteroid( 1.0, 128, Vec2(  512.0, 512.0 ) )
	Dim As RigidBody Ptr Asteroid2 = CreateAsteroid( 1.0, 128, Vec2(  512.0, 512.0 ) )
	
	''
	Dim As Planet Ptr P1 = CreatePlanet( 1E10, 0.0 , 1000.0)

	
	P1->SetPosition( Vec2( 0.0, 0.0 ) )
	
	P1->SetColour( RGB( 80, 128, 64 ) )
	
	P1->SetAngularVelocity( 0.01 )

	''
	Dim As Rocket Ptr R1 = CreateRocket( 100.0, 1000000.0, 0.1, Vec2( 2000.0, 2000.0 ) )
	...
	Dim As KeplerOrbit Ptr K3 = GameWorld.CreateKeplerOrbit( P1, G1 )
	
	Dim As KeplerOrbit K4 = KeplerOrbit( P1, R1 )
	
	''
	k1->ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0,  8000, 0.0 * TWO_PI, 0.0 * TWO_PI, CCW )
	k2->ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0,  8000, 0.0 * TWO_PI, 0.2 * TWO_PI, CCW )
	k3->ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0,  8000, 0.0 * TWO_PI, 0.1 * TWO_PI, CCW )

	k4.ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0, 6000, 0.4 * TWO_PI, 0.0 * TWO_PI, CCW )

	''
	K1->ComputePeriod()
	K2->ComputePeriod()
	'
	Asteroid1->AddAngularVelocity( TWO_PI / K1->Period )
	Asteroid2->AddAngularVelocity( TWO_PI / K2->Period )
	
	G1->AddAngularVelocity( TWO_PI / K1->Period )
	
	'GameWorld.CreateFixedSpring( 0.1, 0.1, 0.1, G1, P1 )
	'GameWorld.CreateLinearSpring( 1.0, 1.0, 1.0, *(G1->LinearStates_[0]), Asteroid1 )
	'GameWorld.CreateLinearSpring( 1.0, 1.0, 1.0, *(G1->LinearStates_[0]), FindNearestRoxel( *(G1->LinearStates_[0]), 1000 ) )
	'GameWorld.CreateLinearSpring( 1.0, 1.0, 1.0, *(G1->LinearStates_[23]), FindNearestRoxel( *(G1->LinearStates_[23]), 1000 ) )
	'GameWorld.CreateLinearSpring( 1.0, 1.0, 1.0, *(G1->LinearStates_[24]), FindNearestRoxel( *(G1->LinearStates_[24]), 1000 ) )
	'GameWorld.CreateLinearSpring( 1.0, 1.0, 1.0, *(G1->LinearStates_.p_back), FindNearestRoxel( *(G1->LinearStates_.p_back), 1000 ) )
...
Sub Game.Puzzle2()
	...
	''
	Dim As Planet Ptr P1 = CreatePlanet( 5E8,, 200.0 )
	Dim As Planet Ptr P2 = CreatePlanet( 3E8,, 200.0 )
	Dim As Planet Ptr P3 = CreatePlanet( 4E8,, 200.0 )
	...
	''
	Dim As Rocket Ptr R1 = CreateRocket( 30.0, 20000.0, 0.2, Vec2( -200.0, 0.0 ) )
	...
	''
	K1->ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0,  3000.0, 0.5 * TWO_PI, -0.25 * TWO_PI, CCW )
	K2->ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0, 12000.0, 0.5 * TWO_PI, 0.5 * TWO_PI,  CW )
...
Sub Game.Puzzle4()
	...
	''
	'' Planets
	Dim As Planet Ptr P1 = CreatePlanet( 1E10, 1000.0 )
	Dim As Planet Ptr P2 = CreatePlanet( 1E8, 200.0 )
	Dim As Planet Ptr P3 = CreatePlanet( 1E8, 200.0 )
	Dim As Planet Ptr P4 = CreatePlanet( 1E8,, 200.0 )
	...
	''
	K1->ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0,  6000, 0.0 * TWO_PI, 0.0  * TWO_PI, CW )
	K2->ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0,  9000, 0.0 * TWO_PI, 0.33 * TWO_PI, CW )
	K3->ApplyOrbit( Vec2( 0.0, 0.0 ), 0.0, 12000, 0.0 * TWO_PI, 0.66 * TWO_PI, CW )
...
Sub Game.Puzzle6()
	...
	Dim As ShapeBody Ptr G1 = CreateShapeGirder( Vec2( 64, 2 ), Vec2( 96, 64 ), X_TRUSS ) 
	...
	Dim As ShapeBody Ptr G14 = CreateArchedShapeGirder( Vec2( 32, 2 ), Vec2( 96.0, 64.0 ), 1.0 * TWO_PI, 600, K_TRUSS )
	...
	G1->SetPosition( Vec2().RandomizeSquare( 3000.0 ) )
	...
	'G12->AddAngularVelocity( 1.0 )
	'G13->AddAngularVelocity( 1.0 )
	G14->AddAngularVelocity( 0.1 )
	...
	'' Rocket
	Dim As Rocket Ptr R1 = CreateRocket( 50.0, 100000.0, 0.2, Vec2( 0.0, 0.0 ) )
...
Sub Game.Puzzle9()
	...
	Puzzle_description_ = "Puzzle: Escape the black hole"
	
	'' Planets
	Dim As Planet Ptr P1 = CreatePlanet( 1E13, 1600.0 )
	Dim As Planet Ptr P2 = CreatePlanet( 1E8,, 200.0 )
