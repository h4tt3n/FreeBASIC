''*******************************************************************************
''
''  Stable and rigid damped Connections using sequential impulses and warmstart
''
...
Type ConnectionType
	...
	As Single _accumulated_impulse
	As Single _rest_impulse
...
Type SimulationType
	...
	As Integer numAngularConnections
...
Sub ConnectionType.ComputeCorrectiveImpulse()
	...
	''
	Dim As Single delta_impulse = Neuron_b->_impulse - Neuron_a->_impulse
	
	Dim As Single impulse_error = delta_impulse - _rest_impulse
	
	Dim As Single corrective_impulse = -impulse_error * reduced_mass
	
	'' apply impulse scaled by mass
	Neuron_a->_impulse -= corrective_impulse * Neuron_a->inverse_mass
	Neuron_b->_impulse += corrective_impulse * Neuron_b->inverse_mass
	
	'' save for warmstart
	_accumulated_impulse += corrective_impulse
...
Sub SimulationType.ComputeReusableData()
	...
	For S As ConnectionType Ptr = ConnectionLo To ConnectionHi
		...
		'' 
		Dim As Single distance = S->Neuron_b->_position - S->Neuron_a->_position
		Dim As Single velocity = S->Neuron_b->_velocity - S->Neuron_a->_velocity
		...
		S->_rest_impulse = -( cstiffness * distance_error * inv_dt + cdamping * velocity_error )
...
Sub SimulationType.ApplyWarmStart()
	...
	For S As ConnectionType Ptr = ConnectionLo To ConnectionHi
		...
		Dim As Single projected_impulse = S->_accumulated_impulse
			
		Dim As Single warmstart_impulse = cwarmstart * projected_impulse
		
		S->Neuron_a->_impulse -= warmstart_impulse * S->Neuron_a->inverse_mass 
		S->Neuron_b->_impulse += warmstart_impulse * S->Neuron_b->inverse_mass
		
		S->_accumulated_impulse = 0.0
...
Sub SimulationType.UpdateScreen()
	...
	For S As ConnectionType Ptr = ConnectionLo To ConnectionHi
		...
		Line(S->Neuron_a->position.x, S->Neuron_a->position.y)-_
			 (S->Neuron_b->position.x, S->Neuron_b->position.y), RGB(0, 255, 0)
	...
	For P As NeuronType Ptr = NeuronLo To NeuronHi
		...
		If ( P->radius > 0.0 ) Then
			...
			Dim As UInteger Col = IIf( P->inverse_mass = 0.0, RGB(160, 160, 160), RGB( P->_Position, 0, 0) )
		...
		Draw String(P->position.x - 20 , P->position.y - 20 ), Str(P->_impulse)
