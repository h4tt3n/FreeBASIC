...
type braintype
	...
	as integer maxnumconnections	= 320
	as integer numneurons					= 100
	as integer numconnections			= 320
...
sub braintype.createbrain()
	...
	if maxnumneurons then 
		...
		for i as integer = 0 to numneurons-1
			...
			neuron[i].psn = vec2f(50 + rnd*(w-100), 50 + rnd*(h-100))
			neuron[i].value = 8 + rnd*32
			neuron[i].firethreshold = 32+rnd*32
			neuron[i].firesignalmag = 32+rnd*24
		next
	end if
		...
		for i as integer = 0 to numconnections-1
			...
			do
				...
				if j <> n then
					...
					if dist < 300^2 then connection[i].receiver = @neuron[j]: exit do
				end if
