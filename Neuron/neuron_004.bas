...
type braintype
	...
	as integer maxnumneurons			= 500
	as integer maxnumconnections	= 2000'320
	as integer numneurons					= 500
	as integer numconnections			= 2000'320
...
sub braintype.createbrain()
	...
	Randomize Timer ''Randomize(42)'' timer
	...
	if maxnumneurons then 
		...
		for i as integer = 0 to numneurons-1
			...
			neuron[i].value = 64'Rnd*128'8 + rnd*96
			neuron[i].firethreshold = 128''+rnd*32 ''32+rnd*32
			neuron[i].firesignalmag = 96''+rnd*32 ''32+rnd*24
		next
	end if
