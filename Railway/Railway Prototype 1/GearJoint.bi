...
Sub GearJointType.ComputeRestImpulse()
	...
	'' state
	Dim As Single angle = body_b->angle - ( body_a->angle * gear_ratio )
	Dim As Single velocity = body_b->angular_velocity - ( body_a->angular_velocity * gear_ratio )
	...
	'' Correction
	rest_impulse = 0.0
	rest_impulse -= IIf( c_stiffness > 0.0 , c_stiffness * angle_error * inv_dt , 0.0 )
	rest_impulse -= IIf( c_damping   > 0.0 , c_damping   * velocity_error       , 0.0 )
...
Sub GearJointType.ApplyCorrectiveImpulse()
	...
	body_b->angular_impulse += corrective_impulse * gear_ratio * body_b->inv_inertia
...
Sub GearJointType.ApplyWarmStart()
	...
	body_b->angular_impulse += warmstart_impulse * gear_ratio * body_b->inv_inertia
