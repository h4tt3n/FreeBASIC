...
Destructor SlideJointType()

End Destructor


''
Sub SlideJointType.ComputeRestImpulse()
	...
	Dim As vec2 ra = body_a->direction_vector.Rotateccw( anchor_a )

	r_b = body_b->direction_vector.Rotateccw( anchor_b )
	
	r_a = ra + n.dot( ( body_b->position + r_b ) - ( body_a->position + ra ) ) * n
	...
	'Dim As Single K2 = body_a->inv_inertia * t.dot( r_a.perpdot( r_a.perpdot( t ) ) )
	'Dim As Single K3 = body_b->inv_inertia * t.dot( r_b.perpdot( r_a.perpdot( t ) ) )
...
Sub SlideJointType.ApplyCorrectiveImpulse()
...
Sub SlideJointType.ApplyWarmStart()
