...
Declare Function ProjectOnAxis(ByVal p0 As vec2f, ByVal p1 As vec2f, ByVal x As vec2f) As float
...
Function ProjectOnAxis(ByVal p0 As vec2f, ByVal p1 As vec2f, ByVal x As vec2f) As float
	
  ''	projects vertex x onto axis normal to line segment (p1 - p0)
...
Function BoxCollision(ByRef A As box_type, ByRef B As box_type) As Integer
	...
	'' box A
	For i As Integer = 0 To A.Num_vertices-1
		
		Dim As Integer j = (i+1) Mod A.Num_vertices
		
		minA = 1000000: maxA = -10000000: minB = 10000000: maxB = -10000000
		
		''	project box A on axis - get min max values
		For k As Integer = 0 To A.Num_vertices-1
			
			Dim As float projection = ProjectOnAxis(A.Psn + A.vertex[i].psn, A.Psn + A.vertex[j].psn, A.Psn + A.vertex[k].psn)
		...
		For k As Integer = 0 To B.Num_vertices-1
			...
			Dim As float projection = ProjectOnAxis(A.Psn + A.vertex[i].psn, A.Psn + A.vertex[j].psn, B.Psn + B.vertex[k].psn)
	...
	'' box B
	For i As Integer = 0 To B.Num_vertices-1
		...
		For k As Integer = 0 To B.Num_vertices-1
			...
			Dim As float projection = ProjectOnAxis(B.Psn + B.vertex[i].psn, B.Psn + B.vertex[j].psn, B.Psn + B.vertex[k].psn)
		...
		For k As Integer = 0 To A.Num_vertices-1
			...
			Dim As float projection = ProjectOnAxis(B.Psn + B.vertex[i].psn, B.Psn + B.vertex[j].psn, A.Psn + A.vertex[k].psn)
