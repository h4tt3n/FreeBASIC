...
''  test # 10 , rectangle collision detection
''
''
''  To-do:
''
...
Const As Integ  Numboxes                = 256                                              ''  number of rigid bodies
...
WindowTitle "Rigid body dynamics with SAT collision # 10"
...
Function ComputeProjection(ByRef Box As BoxType, ByRef A0 As vec2f, ByRef A1 As vec2f) As ProjectionType
...
Function BoxBoxSAT(ByRef BoxA As BoxType, ByRef BoxB As BoxType) As Integer
    ...
    For i As Integer = 0 To 1'BoxA.Num_vertices - 1
        ...
        ProjA2 = ComputeProjection(BoxB, BoxA.Psn + BoxA.vertex[j].psn, BoxA.Psn + BoxA.vertex[i].psn)
    ...
    For i As Integer = 0 To 1'BoxB.Num_vertices - 1
        ...
        ProjB1 = ComputeProjection(BoxA, BoxB.Psn + BoxB.vertex[j].psn, BoxB.Psn + BoxB.vertex[i].psn)
        
        'ComputeMinMax(BoxB, BoxB.Psn + BoxB.vertex[j].psn, BoxB.Psn + BoxB.vertex[i].psn, MinB, MaxB)
        ProjB2 = ComputeProjection(BoxB, BoxB.Psn + BoxB.vertex[j].psn, BoxB.Psn + BoxB.vertex[i].psn)
...
Function BoxPointSAT(ByRef B As BoxType, ByRef P As vec2f) As Integer
