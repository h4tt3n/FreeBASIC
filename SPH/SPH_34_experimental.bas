...
Const As float   k_neighbor         = 2000                         ''   local stiffnes coefficient
Const As float   mju                = 0                         ''   viscosity coefficient
Const As float   mju2               = 0                         ''   viscosity coefficient
Const As float   mju3               = 000                        ''   viscosity coefficient
Const As float   rest_density       = 25.0                         ''   rest density
...
Const As Integer max_neighbors      = 16                           ''   max number of neighbors per particles
...
Sub broad_phase_collision()
  ...
  For P As ParticleType Ptr = Lo To Hi
    ...
    Dim As UShort cell_col = P->Psn.y * inv_cell_hgt

    If (cell_col < 1) Or (cell_col > num_cells_col) Then Continue For

    Dim As UShort cell_row = P->Psn.x * inv_cell_wid
...
Sub calculate_internal_force()
	...
	For N As NeighborType Ptr = @neighbor(1) To @neighbor(num_neighbors)
		...
		If N->distance_squared > zero Then
			...
			Dim As Vec2f F = N->n_over_distance * ( _
				...
				( rest_density - ( N->A->density + N->B->density) ) * 4000.0 +  _	''	4000
				( N->A->vel - N->B->vel ).dot( N->n_over_distance ) * 50.0	)		''	50
	...
	'( rest_density - N->density )                      * 4000.0 +  _	''	4000
...
Sub draw_particles()
   ...
   For P As ParticleType Ptr = Lo To Hi
     ...
     If col_b < 0 Then col_b = 0
		...
		Line(P->psn.x-5, P->psn.y-5)-(P->psn.x+5, P->psn.y+5), RGB(col_r, col_g, col_b), bf
