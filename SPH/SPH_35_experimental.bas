...
Const As Single  rest_r					= 27.5
Const As Single  rest_r_sqrd		   = rest_r * rest_r


'Const As float   rest_density       = 30.0                         ''   rest density
Const As float   rest_density       = 27500.0                         ''   rest density
  ...
  For x As Integer = 1 To block_width
    For y As Integer = 1 To block_height
      ...
      With Particle(n)
        ...
        .psn.x = border + r + 1 * 0.5 + x * r * 0.5
        '.psn.x = border + r + ( x * rest_r )
        .psn.y = screen_hgt - border - r - 1 * 0.5 - y * r * 0.5
        '.psn.y = screen_hgt - ( border + r ) - ( y * rest_r )
...
Sub calculate_internal_force()
  ...
  For N As NeighborType Ptr = @neighbor(1) To @neighbor(num_neighbors)
    ...
    'Dim As Single F_pressure

    'Dim As Single F_viscosity

    'Dim As Single vel


    '  inv_dens_sum_sqrd = 1.0 / (N->A->density * N->A->density + N->B->density * N->B->density)
    '  'vel_over_dens = vel / (N->A->density + N->B->density)
    '  vel_over_dens = vel * inv_dens_sum_sqrd
    '  F_viscosity = mju * vel_over_dens * N->density + mju2 * vel_over_dens * N->density2 + mju3 * vel_over_dens * N->density3
    
      ''   velocity vector projected onto distance vector
   Dim As Single vel = ( N->B->vel - N->A->vel ).dot( N->n_x_inv_dist )
   'Dim As Single vel = ( N->B->vel - N->A->vel ).dot( ( N->B->psn - N->A->psn ) )
    
    
   If N->distance_squared > zero Then

    ''   force vector
    'Dim As Vec2f F = N->n_x_inv_dist * (F_pressure + F_viscosity)
    
    Dim As Vec2f F = N->n_x_inv_dist * ( _
    							( rest_r_sqrd - N->distance_squared )                * 50.0 -  _	''	50
    						   ( rest_density - ( N->A->density + N->B->density ) ) * 4.0  -  _	''	4
    						 	  vel                                                * 50.0	)		''	50
    						 	  
        						'( rest_r_sqrd - N->distance_squared )                * 100.0   -  _	''	50
        						'( rest_density - N->density )                        * 10000.0    +  _	''	1000
        						'( 20.0 - ( N->A->density + N->B->density) * 0.5 )    * 4000.0 +  _	''	4000
        						'( rest_density - N->density )                        * 100.0   -  _	''	50
...
Sub draw_particles()
   ...
   For P As ParticleType Ptr = Lo To Hi
     ...
     Dim As float col_r = P->density * color_density_scale
     Dim As float col_g 
     Dim As float col_b 
     ...
     If col_b < 0 Then col_b = 0
		...
		Line(P->psn.x-1, P->psn.y-1)-(P->psn.x+1, P->psn.y+1), RGB(col_r, col_g, col_b), bf
   ...
   Locate 2, 2: Print particle(1).density
...
Sub Check_if_neighbor(ByVal A As ParticleType Ptr, ByVal B As ParticleType Ptr)
  ...
  'N->density  = 1.0 - distance_squared * inv_r2_sqrd
  N->density  = r2_sqrd - distance_squared
  'N->density  = density
  'N->density2 = density * density
  'N->density3 = density * N->density2

  If distance_squared > zero Then
  	
    N->n_x_inv_dist = distance_vector / distance_squared
    
  Else
  	
    N->n_x_inv_dist = vec2f(0.0, 0.0)
...
Sub MouseType.Update
   ...
   If (Btn = 1) Then
     ...
     For i as Integer = 1 to numparticles
       ...
       If vel.x <> 0.0 Then Particle(i).frc.x -= ( (Particle(i).vel.x - vel.x) * inv_dt ) * ( 1 - dst_Sqrd / int_radius_sqrd ) * 0.15
       If vel.y <> 0.0 Then Particle(i).frc.y -= ( (Particle(i).vel.y - vel.y) * inv_dt ) * ( 1 - dst_Sqrd / int_radius_sqrd ) * 0.15
   ...
   If whl > whl_old Then
     ...
     For i as Integer = 1 to numparticles
       ...
       If Abs(dst.x) < 1.0 then continue for
       If Abs(dst.y) < 1.0 then continue For
       
       Dim As float dst_Sqrd = dst.x*dst.x+dst.y*dst.y
       
       Particle(i).frc.x +=  dst.y / dst_Sqrd * 300000
       Particle(i).frc.y -=  dst.x / dst_Sqrd * 300000
