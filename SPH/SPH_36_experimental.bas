...
Const As Single  PARTICLE_RADIUS            = 30.0                           ''   particle interaction radius
Const As Single  PARTICLE_DIAMETER          = PARTICLE_RADIUS * 2.0                           ''   particle interaction radius
Const As Single  INV_PARTICLE_RADIUS        = 1.0 / PARTICLE_RADIUS                        ''   particle interaction radius
Const As Single  PARTICLE_RADIUS_SQRD       = PARTICLE_RADIUS * PARTICLE_RADIUS                        ''  radius squared
Const As Single  PARTICLE_DIAMETER_SQRD     = PARTICLE_DIAMETER * PARTICLE_DIAMETER                        ''  radius squared
Const As Single  INV_PARTICLE_RADIUS_sqrd   = 1.0 / PARTICLE_RADIUS_SQRD                   ''  inverse radius squared
Const As Single  INV_PARTICLE_DIAMETER_SQRD = 1.0 / PARTICLE_DIAMETER_SQRD                   ''  inverse radius squared

Const As Single  REST_DISTANCE              = 55.0 '29.0
Const As Single  REST_DISTANCE_SQRD         = REST_DISTANCE * REST_DISTANCE

Const As float   REST_DENSITY               = 160000.0                        ''   rest density_
Const As float   color_density_Scale        = 255 / rest_density

Const As Integer block_wiDTh        = 40                          ''
Const As Integer block_height       = 40                          ''
...
Const As Integer screen_wid         = 600                         ''   screen wiDTh
...
Sub calculate_internal_force()
  ...
  For N As NeighborType Ptr = @neighbor(1) To @neighbor(num_neighbors)
    ...
    If N->distanceSquared_ > ZERO Then
      ...
      ''   force vector
      'Dim As Vec2f F = N->nOverDistance_ * (F_pressure + F_viscosity)
      Dim As Vec2f force = N->nOverDistance_ * ( _ 
          N->density_                                                            * 40.0   -  _  ''  40000
        ( REST_DENSITY - ( N->particleA_->density_ + N->particleB_->density_) )  * 1.0 +  _  ''  4000
          velocity_                                                              * 100.0  )    ''  50
...
Sub Integrate()
  ...
  ''   midpoint / leapfrog
  'For P As ParticleType Ptr = Lo To Hi
  '    P->velocity_ += P->force_ * HALF_DT
  '    P->position_ += P->velocity_ * DT + P->force_ * HALF_DT_SQRD
  '    P->velocity_ += P->force_ * HALF_DT
  'Next
  ...
  '  'P->position_ = 2.0 * P->position_ - P->prevPosition_ + P->force_ * DT_SQRD
...
Sub draw_particles()
  ...
  For P As ParticleType Ptr = Lo To Hi
    ...
    'Circle(P->position_.x, P->position_.y), r, RGBA(col_r, col_g, col_b, 128),,, 1, f
    'Circle(P->position_.x, P->position_.y), r, RGBA(255, 255, 255, 16),,, 1, f
    'Circle(P->position_.x, P->position_.y), 2, RGB (255, 0, 0),,, 1, f
    'pset(P->position_.x, P->position_.y), RGB(Col, 0, 255 - col)
    'pset(.position_.x, .position_.y), RGB(0, 0, 0)

  Next

  'For P As ParticleType Ptr = Lo To Hi
  '
  '  'Dim As float col = P->density_ * 25

  '   'Circle(P->position_.x, P->position_.y), 4, RGBA(col, 0.0, 255 - col, 64),,, 1, f
  '   'Line(P->position_.x-1, P->position_.y-1)-(P->position_.x+1, P->position_.y+1), RGB(col, 0, 255 - col), bf
  '   Line(P->position_.x-1, P->position_.y-1)-(P->position_.x+1, P->position_.y+1), RGB(255, 255, 255), bf
  '   'Circle(P->position_.x, P->position_.y), 2, RGB (255, 0, 0),,, 1, f
  '   'pset(P->position_.x, P->position_.y), RGB(Col, 0, 255 - col)
  '   'pset(.position_.x, .position_.y), RGB(0, 0, 0)
  '
...
Sub Check_if_neighbor(ByVal A As ParticleType Ptr, ByVal B As ParticleType Ptr)
  ...
  ''  density_ kernel W(Xij, r) = ( 1 - Xij^2 / r^2 )
  'Dim As Single density_ = 1.0 - distanceSquared_ * INV_PARTICLE_DIAMETER_SQRD
  ...
  'N->density_  = 1.0 - distance_squared * INV_PARTICLE_DIAMETER_SQRD
