...
Const As Single  LOCAL_PRESSURE_COEFF   = 0.01 '' 0.0004                                 ''                              ''
Const As Single  GLOBAL_PRESSURE_COEFF  = 0.00	  '' 0.03                                  ''
Const As Single  LOCAL_DAMPING_COEFF    = 0.1	  '' 0.4                                 ''
...
Sub FluidType.ComputeCorrectiveImpulse()
	...
	For PP As ParticlePairType Ptr = @particlePairs( 1 ) To @particlePairs( numParticlePairs_ )
		...
		'' impulse error
		Dim As Single impulse_error = PP->unit_.dot( delta_impulse ) - PP->rest_impulse_
		
		'' corrective impulse
	   Dim As Vec2 corrective_impulse = impulse_error * PP->unit_
...
Sub WorldType.checkPair( ByVal A As ParticleType Ptr, ByVal B As ParticleType Ptr )
	...
	''
	PP->unit_ = distance_vector / Sqr( distance_squared )
...
Sub FluidType.ComputeReusableData()
	...
	For N As ParticlePairType Ptr = @particlePairs( 1 ) To @particlePairs( numParticlePairs_ )
		...
		'N->velocity_ = N->unit_.dot( N->particleB_->velocity_ - N->particleA_->velocity_ )
		
		Dim As Single local_pressure_impulse  = N->density_ * localPressureCoefficient_
		'Dim As Single local_pressure_impulse  = N->density_ * N->density_ * localPressureCoefficient_
		...
		Dim As Single local_damping_impulse   = - N->unit_.dot( N->particleB_->velocity_ - N->particleA_->velocity_ ) * localDampingCoefficient_ 
