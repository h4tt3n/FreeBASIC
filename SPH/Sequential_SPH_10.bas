...
Const As Single  MIN_DISTANCE           = 4.0                                       '' minimum distance
Const As Single  PI                     = 4.0 * Atn( 1.0 )                             ''	pi
Const As Single  G                      = 10.0                                         ''

Const As Integer  ITERATIONS            = 10                                       ''
...
Const As Single  LOCAL_PRESSURE_COEFF   = 0.0004  '' 0.0004                                 ''
'Const As Single  LOCAL_PRESSURE_COEFF   = 0.4	  '' 0.4                                  ''
Const As Single  GLOBAL_PRESSURE_COEFF  = 0.04	  '' 0.03                                  ''
Const As Single  LOCAL_DAMPING_COEFF    = 0.4	  '' 0.4                                 ''
...
Const As Integer MAX_PAIRS_LOCAL        = 12                                           '' max number of neighbors per particles
...
Type FluidType
	...
	Declare Constructor( ByVal _globalpressure As Single, _
		                  ByVal _localpressure  As Single, _
		                  ByVal _localdamping   As Single, _
		                  ByVal _particleradius As Single, _
		                  ByVal _restdistance   As Single, _
		                  ByVal _restdensity    As Single )
...
Constructor Fluidtype( ByVal _globalpressure As Single, _
		                 ByVal _localpressure  As Single, _
		                 ByVal _localdamping   As Single, _
		                 ByVal _particleradius As Single, _
		                 ByVal _restdistance   As Single, _
	...
	''
	This.globalPressureCoefficient_ = _globalpressure
	This.localDampingCoefficient_   = _localdamping
	This.localPressureCoefficient_  = _localpressure
	This.particleRadius_            = _particleradius
	This.restDistance_              = _restdistance
	This.restDensity_               = _restdensity
	
	''
	This.restDistanceSquared_       = This.restDistance_^ 2.0
	This.particleDiameter_          = This.particleRadius_ * 2.0
	This.particleDiameterSquared_   = This.particleDiameter_ ^ 2.0
	
	''
	This.firstParticle_  = @This.particles ( 1 )
	This.lastParticle_   = @This.particles ( MAX_PARTICLES )
...
Sub SimulationType.initiateSimulation()
	...
	fluid = FluidType( GLOBAL_PRESSURE_COEFF, _
...
Sub SimulationType.computeBroadPhase( ByVal F As FluidType Ptr, ByVal G As GridType Ptr  )
	...
	For P As ParticleType Ptr = F->firstParticle_ To F->lastParticle_
		...
		Dim As UShort cell_col = Cast( UShort, P->position_.x * G->invCellWidth_ )
	
		If ( cell_col < 0 ) Or ( cell_col > G->numCellsX_ ) Then Continue For
	
		Dim As UShort cell_row = Cast( UShort, P->position_.y * G->invCellHeight_ )
...
Sub SimulationType.ComputeReusableData()
	...
	For N As ParticlePairType Ptr = @fluid.particlePairs( 1 ) To @fluid.particlePairs( fluid.numParticlePairs_ )
		...
		N->velocity_ = N->nOverDistance_.dot( N->particleB_->velocity_ - N->particleA_->velocity_ )
		
		'Dim As Single local_pressure_impulse  = N->density_ * fluid.localPressureCoefficient_
		Dim As Single local_pressure_impulse  = N->density_ * N->density_ * fluid.localPressureCoefficient_
