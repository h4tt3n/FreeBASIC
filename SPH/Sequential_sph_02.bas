...
Const As Single  LOCAL_PRESSURE_COEFF   = 0.2	                                       ''
Const As Single  GLOBAL_PRESSURE_COEFF  = 0.01	                                       ''
Const As Single  LOCAL_DAMPING_COEFF    = 0.4                                        
Const As Single  WARMSTART_COEFF        = 10.0                                        

Const As Single  REST_DISTANCE          = 28.0    '26                                  ''

Const As Single  REST_DENSITY           = 11000.0 '12000                               '' rest pressure
Const As Single  COLOR_PRESSURE_SCALE   = 255 / REST_DENSITY                           ''

Const As Integer BLOCK_WID              = 30                                           ''
Const As Integer BLOCK_HGT              = 70                                          ''
...
Const As Integer SCREEN_HGT             = 1000                                          '' screen height
Const As Integer SCREEN_WID             = 300                                         '' screen wiDTh
...
Const As Single  BORDER_DAMPING         = 0.25                                        ''
...
Sub SimulationType.RunSimulation()
	...
	Do
		...
		If MultiKey( FB.SC_SPACE ) Then 
			...
			fluid.ComputeWarmStart()
			
		Else
			
			fluid.ClearWarmstart()
...
Sub FluidType.ComputeCorrectiveImpulse()
	...
	For PP As ParticlePairType Ptr = @particlePairs( 1 ) To @particlePairs( numParticlePairs_ )
	   ...
	   Dim As Vec2 corrective_impulse = impulse_error * PP->nOverDistance_
		...
		PP->accumulated_impulse_ += corrective_impulse
	
	Next
	
End Sub

Sub FluidType.ComputeWarmstart()
	
	For PP As ParticlePairType Ptr = @particlePairs( 1 ) To @particlePairs( numParticlePairs_ )
		
		Dim As Single projected_impulse = PP->nOverDistance_.dot( PP->accumulated_impulse_ )
		
		If ( projected_impulse < 0.0 ) Then
			
			Dim As Vec2 warmstart_impulse = WARMSTART_COEFF * projected_impulse * PP->nOverDistance_
			
			PP->particleA_->impulse_ += warmstart_impulse
			PP->particleB_->impulse_ -= warmstart_impulse
			
		EndIf
		
		PP->accumulated_impulse_ = Vec2( 0.0, 0.0 )
