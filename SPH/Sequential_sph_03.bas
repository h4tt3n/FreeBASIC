...
'Const As Single  LOCAL_PRESSURE_COEFF   = 0.00008	  '64                                  ''
Const As Single  LOCAL_PRESSURE_COEFF   = 0.0004	  '64                                  ''
Const As Single  GLOBAL_PRESSURE_COEFF  = 0.03	  '4                                   ''
Const As Single  LOCAL_DAMPING_COEFF    = 0.1	  '64                                  ''

Const As Single  REST_DISTANCE          = 30.0    '26                                  ''
'Const As Single  REST_DISTANCE_SQD      = REST_DISTANCE * REST_DISTANCE                ''

Const As Single  REST_DENSITY           = 14000.0 '12000                               '' rest pressure
	...
	For x As Integer = 1 To BLOCK_WID
		...
		For y As Integer = 1 To BLOCK_HGT
			...
			With fluid.particles (n)
				...
				.position_.x = BORDER + PARTICLE_RADIUS * 0.8 + x * PARTICLE_RADIUS * 0.8
				.position_.y = SCREEN_HGT - BORDER - PARTICLE_RADIUS * 0.8 - y * PARTICLE_RADIUS * 0.8
	...
	ScreenLock
		...
		If renderMethod_ = 2 Then
			...
			For P As ParticleType Ptr = fluid.firstParticle_ To fluid.lastParticle_
				...
				Line( P->position_.x-2, P->position_.y-2 )-( P->position_.x+2, P->position_.y+2 ), RGB(col_r, col_g, col_b), bf
...
Sub SimulationType.checkPair( ByVal A As ParticleType Ptr, ByVal B As ParticleType Ptr )
	...
	If distance_squared > MIN_DISTANCE Then
		...
		'PP->nOverDistance_ = distance_vector / Sqr( distance_squared )
