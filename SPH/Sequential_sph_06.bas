...
Const As Single  LOCAL_PRESSURE_COEFF   = 0.0002	  '64                                  ''
Const As Single  GLOBAL_PRESSURE_COEFF  = 4.0	  '4                                   ''
Const As Single  LOCAL_DAMPING_COEFF    = 2.0	  '64                                  ''
...
Const As Integer MAX_PARTICLES_PER_CELL = 24                                          '' max number of particles held by one cell
...
Sub WorldType.computeBroadPhase( ByVal F As FluidType Ptr, ByVal G As GridType Ptr  )
	...
	For P As ParticleType Ptr = F->firstParticle_ To F->lastParticle_
		...
		Dim As Ubyte cell_col = Cast( Ubyte, P->position_.x * G->invCellWidth_ )
	
		If ( cell_col < 0 ) Or ( cell_col > G->numCellsX_ ) Then Continue For
	
		Dim As UByte cell_row = Cast( UByte, P->position_.y * G->invCellHeight_ )
