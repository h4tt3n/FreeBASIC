...
Const As Integer ITERATIONS             = 4                                   '' <--- Change if running slow !!!

Const As Single  PARTICLE_RADIUS        = 15.0                                          '' particle radius
Const As Single  PARTICLE_DIAMETER      = PARTICLE_RADIUS * 2.0                        ''

Const As Single  LOCAL_PRESSURE_COEFF   = 0.0002  '' 0.00015                                ''                              ''
Const As Single  GLOBAL_PRESSURE_COEFF  = 0.018	  '' 0.02                                  ''
Const As Single  LOCAL_DAMPING_COEFF    = 0.3	  '' 0.8                                 ''

Const As Single  REST_DISTANCE          = 30.0    '' 30                                  ''

Const As Single  REST_DENSITY           = 10000.0 '12000                               '' rest pressure
Const As Single  COLOR_PRESSURE_SCALE   = 255 / ( REST_DENSITY * 0.6 )                          ''

Const As Integer BLOCK_WID              = 100                                       '' <--- Change if running slow !!!
Const As Integer BLOCK_HGT              = 20                                        '' <--- Change if running slow !!!
...
Sub WorldType.RunWorld()
	...
	Scr.CreateScreen( SCREEN_WID, SCREEN_HGT )
	...
	Do
		...
		'' Iterations
		If ( MultiKey( fb.SC_I ) And MultiKey( fb.SC_UP   ) ) Then iterations_ += 1
		If ( MultiKey( fb.SC_I ) And MultiKey( fb.SC_DOWN ) ) Then iterations_ -= 1
		
		If iterations_ < 1 Then iterations_ = 1
		...
		''
		For i As Integer = 1 To Iterations_
...
'				Line(P->prevPosition_.x, P->prevPosition_.y)-(P->position_.x, P->position_.y), RGBA(col_r, col_g, col_b, 192)
	...
	If renderMethod_ = 2 Then
		...
		For P As ParticleType Ptr = fluid.firstParticle_ To fluid.lastParticle_
			...
			'Pset( P->position_.x, P->position_.y ), RGB(255, 255, 255)
	...
	Locate 6, 2: Print "iterations:" ; iterations_
...
Sub FluidType.ComputeReusableData()
	...
	For N As ParticlePairType Ptr = @particlePairs( 1 ) To @particlePairs( numParticlePairs_ )
		...
		Dim As Single global_pressure_impulse = ( ( N->particleA_->density_ + N->particleB_->density_) - restDensity_ ) * globalPressureCoefficient_ 
...
Sub ScreenType.CreateScreen( w As Integer, h As Integer )

	ScreenRes( w, h, 24, 2, fb.GFX_ALPHA_PRIMITIVES )
...
Sub WorldType.UpdateMouse
	...
	If ( mouse.button_ = 1 ) Then
		...
		For P As ParticleType Ptr = fluid.firstParticle_ To fluid.lastParticle_
			...
			P->impulse_ -= ( ( P->velocity_ - mouse.velocity_ ) * INV_DT ) * ( 1.0 - distance_squared / INTERACT_RADIUS_SQD ) * 0.004
	...
	If ( mouse.button_ = 2 ) Then
		...
		For P As ParticleType Ptr = fluid.firstParticle_ To fluid.lastParticle_
			...
			P->impulse_ += dst / distance_squared * 2500.0
