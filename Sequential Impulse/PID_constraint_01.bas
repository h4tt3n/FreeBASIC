''*******************************************************************************
''
''  PID-style rigid constraint
''
''  Version 0.1, November 2019, Michael "h4tt3n" Nissen
''
''  Removed sqr() from spring equations
''
''  Improved angular springs by removing mass from equations and using inverse and reduced mass instead.
''  Optimized by removing spring state vectors, and particle mass, freeing a single pr. particle and two Vec2 pr. spring.
''
''
''  Controls:
''  
...
Const As Single  density            = 1.0              ''  ball density
...
Sub SimulationType.RunSimulation()
	...
	''
	Dim As Single delta_angle  =  -0.00001 * 2 * PI
...
Sub SimulationType.Demo2()
	...
	'Dim As Integer num_angular_Springs = 106
	...
	For i As integer = 1 To num_Particles
		...
		P->position = 0.5 * Vec2( screen_wid, screen_hgt * 0.5 ) + Vec2( Cos(Angle), Sin(Angle) ) * SpringLength
		
		'P->position = 0.5 * Vec2( screen_wid, screen_hgt ) + 0.125 * Vec2( screen_wid * ( Rnd - Rnd ) , screen_hgt * ( Rnd - Rnd ) )
...
Sub SimulationType.ComputeReusableData()
	...
	For S As SpringType Ptr = SpringLo To SpringHi
		...
		''
		'S->distance_error_acc += distance_error
		'S->distance_error_acc *= 0.5
		
		'' impulse needed to satisfy the constraint
		S->rest_impulse = -( cP * distance_error * inv_dt + cI * S->distance_error_acc * inv_dt + cD * velocity_error  )
	...
	'	
	'	If ( inverse_inertia > 0.0 ) Then
	'		
	'		A->reduced_inertia = 1.0 / inverse_inertia
	'		
	'	ElseIf ( A->spring_a->inverse_inertia = 0.0 ) Then
	'		
	'		A->reduced_inertia = A->spring_b->inverse_inertia
	'		
	'	ElseIf ( A->spring_b->inverse_inertia = 0.0 ) Then
	'		
	'		A->reduced_inertia = A->spring_a->inverse_inertia
	'	
	'	Else
	'		
	'		A->reduced_inertia = 0.0
	'		
	'	EndIf
	'	
...
Sub SimulationType.ApplyWarmStart()
	...
	For S As SpringType Ptr = SpringLo To SpringHi
		...
		If ( projected_impulse < 0.0 ) Then
			...
			Dim As Vec2 warmstart_impulse = cW * projected_impulse * S->unit
...
Sub SimulationType.UpdateScreen()
	...
	Locate  8, 2: Print Using "(T) Iterations ###"; iterations
	Locate 12, 2: Print Using "P: #.####"; cP
	Locate 14, 2: Print Using "I: #.####"; cI
	Locate 16, 2: Print Using "D: #.####"; cD
	
	If ( warmstart = 0 ) Then 
		
		Locate 18, 2: Print "(W) Linear warmstart  OFF"
		
	Else
		
		Locate 18, 2: Print Using "(W) Linear warmstart  #.##"; cW
	...
	For S As SpringType Ptr = SpringLo To SpringHi
		...
		'Dim As Vec2 position = ( S->particle_a->position * S->particle_a->inverse_mass + _
		'                         S->particle_b->position * S->particle_b->inverse_mass ) * S->reduced_mass
		'
		'Circle( position.x, position.y ), 2.2, RGB( 160, 160, 160 ),,, 1, f
...
Sub SimulationType.UpdateInput()
	...
	If ( ScreenEvent( @e ) ) Then
		...
		Case fb.EVENT_KEY_PRESS
			...
			'' Iterations
			If ( MultiKey( fb.SC_T ) And ( e.scancode = fb.SC_UP   ) ) Then iterations += 1
			If ( MultiKey( fb.SC_T ) And ( e.scancode = fb.SC_DOWN ) ) Then iterations -= 1
			
			'' 
			If ( MultiKey( fb.SC_P ) And ( e.scancode = fb.SC_UP   ) ) Then cP += 0.01
			If ( MultiKey( fb.SC_P ) And ( e.scancode = fb.SC_DOWN ) ) Then cP -= 0.01
			
			'' 
			If ( MultiKey( fb.SC_I ) And ( e.scancode = fb.SC_UP   ) ) Then cI += 0.0001
			If ( MultiKey( fb.SC_I ) And ( e.scancode = fb.SC_DOWN ) ) Then cI -= 0.0001
			
			'' 
			If ( MultiKey( fb.SC_D ) And ( e.scancode = fb.SC_UP   ) ) Then cD += 0.01
			If ( MultiKey( fb.SC_D ) And ( e.scancode = fb.SC_DOWN ) ) Then cD -= 0.01
			
			'' 
			If ( MultiKey( fb.SC_W ) And ( e.scancode = fb.SC_UP   ) ) Then cW += 0.01
			If ( MultiKey( fb.SC_W ) And ( e.scancode = fb.SC_DOWN ) ) Then cW -= 0.01
	...
	If cP < 0.0 Then cP = 0.0
	If cP > 1.0 Then cP = 1.0
	
	If cI < 0.0 Then cI = 0.0
	If cI > 1.0 Then cI = 1.0
	
	If cD < 0.0 Then cD = 0.0
	If cD > 1.0 Then cD = 1.0
	
	If cW < 0.0 Then cW = 0.0
	If cW > 1.0 Then cW = 1.0
