...
Const As float Angular_Stiffnes  = 0.5          '' angular spring stiffnes
Const As float Angular_damping   = 0.25         '' angular spring damping
  ...
  With Body[i]
   ...
   For j As integ = 0 To .NumAngularSprings-1
    With .AngularSpring[j]
     ...
     'Dim As float Torque  = -( (Angular_Stiffnes/(dt*dt)) * Sin_Angle + (Angular_Damping/dt)*Angular_Vel) /_
