...
''
''  Version 0.14, april 2016, Michael "h4tt3n" Nissen
''
''  Controls:
''  
''  Demos                      :  F1 - F5
...
Type SimulationType
	...
	As ParticleType particle ( 0 To max_particles )
	As SpringType   spring   ( 0 To max_springs )
...
Sub SimulationType.Demo5()
	...
	Dim As Integer num_Particles = 101
	Dim As Integer num_Springs   = 100
...
Sub SimulationType.ComputeRestforce()
	...
	'' Compute the force needed to satisfy the constraint in one loop
...
Sub SimulationType.ApplyCorrectiveforce()
	...
	For S As SpringType Ptr = SpringLo To SpringHi
		...
		Dim As Vec2 current_force = S->particle_b->force - S->particle_a->force
		
		Dim As Single force_error = S->unit.dot( current_force ) - S->rest_force
...
Sub SimulationType.ComputeState()
	...
	''	Compute new state vectors with symplectic Euler algorithm
...
Sub SimulationType.CreateScreen( ByVal Wid As Integer, ByVal Hgt As Integer )
   ...
   WindowTitle "Mike's sequential force demo. See source for controls."
