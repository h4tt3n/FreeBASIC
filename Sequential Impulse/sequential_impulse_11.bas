...
Const As Integer num_iterations = 8                ''  number of impulse iterationsations per state update
Const As Integer num_particles  = 100                ''  number of particles
...
Sub SimulationType.CreateSimulation()
	...
	For P As ParticleType Ptr = ParticleLo To ParticleHi
		...
		P->mass         = 1.0'Abs( min_mass + ( Rnd() * ( max_mass ^ 4.0 ) ) ^ (1.0 / 4.0 ) )
		P->inverse_mass = 1.0 / P->mass
		P->radius       = ( ( P->mass / density ) / (4/3) * pi ) ^ (1/3) 
		P->position.x   = 0.5 * screen_wid + ( Rnd() - Rnd() ) * 1000'00
		P->position.y   = 0.5 * screen_hgt + ( Rnd() - Rnd() ) * 1000'00
		
		n += 1
		
		If P = ParticleLo Then P->inverse_mass = 0.0: P->position = Vec2( 0.5 * screen_wid, 0.1 * screen_hgt )
