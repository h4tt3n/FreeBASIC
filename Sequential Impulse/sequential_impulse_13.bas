...
''
''  Version 0.13, march 2016, Michael "h4tt3n" Nissen
''
...
Sub SimulationType.ComputeRestimpulse()
	...
	For S As SpringType Ptr = SpringLo To SpringHi
		...
		S->rest_impulse = -( cstiffness * distance_error * inv_dt + cdamping * velocity_error ) * S->reduced_mass
...
Sub SimulationType.ApplyCorrectiveimpulse()
	...
	For S As SpringType Ptr = SpringLo To SpringHi
	   ...
	   Dim As Vec2 corrective_impulse = -impulse_error * S->unit
		
		S->particle_a->impulse -= corrective_impulse' * S->particle_a->inverse_mass' * S->particle_a->mass 
		S->particle_b->impulse += corrective_impulse' * S->particle_b->inverse_mass' * S->particle_b->mass 
...
Sub SimulationType.ApplyWarmStart()
	...
	'		
	'		S->particle_a->impulse -= warmstart_impulse' * S->particle_a->inverse_mass' * S->particle_a->mass 
	'		S->particle_b->impulse += warmstart_impulse' * S->particle_b->inverse_mass' * S->particle_b->mass 
	'		
