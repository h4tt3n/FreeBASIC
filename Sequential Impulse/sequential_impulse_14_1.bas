...
''
''  Version 0.14.1, november 2016, Michael "h4tt3n" Nissen
''
''  Controls:
''  
''  Demos                      :  F1 - F6
	...
	Do
		...
		For P As ParticleType Ptr = ParticleLo To ParticleHi
			...
			If ( P->inverse_mass > 0.0 ) Then
				...
				P->velocity += Vec2( 0.0, inv_dt * gravity )
...
Sub SimulationType.Demo3()
	...
	for i as integer = 1 to GirderWidth - 1
		For j as integer = 1 to GirderLength-1 
			...
			Dim As SpringType Ptr S1 = CreateSpring()
			
			S1->particle_a = @particle(i+(j-1)*GirderWidth)
			S1->particle_b = @particle(j*GirderWidth+i+1)
			
			Dim As SpringType Ptr S2 = CreateSpring()
			
			S2->particle_a = @particle( j*GirderWidth + i )
			S2->particle_b = @particle( (j-1)*GirderWidth + i+1)
...
Sub SimulationType.Demo5()
	...
	For i As Integer = 2 To num_Particles
		...
		P->velocity.x   = ( Rnd() - Rnd() ) * state * 0.1 * inv_dt
		P->velocity.y   = ( Rnd() - Rnd() ) * state * 0.1 * inv_dt
