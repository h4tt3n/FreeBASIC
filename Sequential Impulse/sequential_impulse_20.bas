...
''
''  Version 0.20, april 2016, Michael "h4tt3n" Nissen
''
...
Sub SimulationType.Demo2()
	...
	For i As integer = 2 To num_Particles
		...
		P->position.x   = 0.5 * screen_wid + ( i - 1 ) * SpringLength
		P->position.y   = 0.2 * screen_hgt + 30 * ( i - 1 )
...
Sub SimulationType.ComputeRestimpulse()
	...
	For S As SpringType Ptr = SpringLo To SpringHi
		...
		'S->inverse_inertia = 1.0 / S->inertia
		S->inverse_inertia = IIf( S->inertia = 0.0 , 0.0 , 1.0 / S->inertia )
	...
	For A As AngularSpringType Ptr = AngularSpringLo To AngularSpringHi
		...
		A->reduced_inertia = 2'( A->spring_a->inertia * A->spring_b->inertia ) / _
		                     '( A->spring_a->inertia + A->spring_b->inertia )
		                     
		'A->reduced_inertia = 1.0 / inverse_inertia
		A->reduced_inertia = IIf( inverse_inertia = 0.0 , 0.0 , 1.0 / inverse_inertia )
		
		A->rest_impulse = -0.1 * angle_error * inv_dt - 0.1 * velocity_error
		'A->rest_impulse = -angle_error * inv_dt * 100 - velocity_error * 100
...
Sub SimulationType.ApplyCorrectiveimpulse()
	...
	For A As AngularSpringType Ptr = AngularSpringLo To AngularSpringHi
		...
		Dim As Single angular_impulse_aa = ( A->spring_a->particle_a->position - A->spring_a->position ).perpdot( A->spring_a->particle_a->impulse )
		Dim As Single angular_impulse_ab = ( A->spring_a->particle_b->position - A->spring_a->position ).perpdot( A->spring_a->particle_b->impulse )
		
		Dim As Single angular_impulse_ba = ( A->spring_b->particle_a->position - A->spring_b->position ).perpdot( A->spring_b->particle_a->impulse )
		Dim As Single angular_impulse_bb = ( A->spring_b->particle_b->position - A->spring_b->position ).perpdot( A->spring_b->particle_b->impulse )
		
		Dim As Single current_angular_impulse = ( angular_impulse_bb + angular_impulse_ba ) - ( angular_impulse_ab + angular_impulse_aa ) 
...
Sub SimulationType.UpdateScreen()
	...
	ScreenLock
		...
		'Locate 17, 2: Print AngularSpringLo->angle.y
