...
''
''  Version 0.27, August 2017, Michael "h4tt3n" Nissen
''
...
Const As Single  gravity            = 10.0 * DT        ''  gravity
...
Sub SimulationType.Demo2()
	...
	Dim As Integer SpringLength        = 80.0
	...
	For S As SpringType Ptr = SpringLo To SpringHi
		...
		Else
		   ...
		   '                S->particle_b->velocity * S->particle_b->mass ) / _
			...
			S->velocity = ( S->particle_a->velocity * S->particle_a->mass + _
		                   S->particle_b->velocity * S->particle_b->mass ) * _
		                   ( S->particle_a->inverse_mass + S->particle_b->inverse_mass )
		...
		Dim As Single inertia = S->local_position_a.lengthsquared() * S->particle_a->mass + _
		                        S->local_position_b.lengthsquared() * S->particle_b->mass
		
		S->inverse_inertia = IIf( inertia > 0.0 , 1.0 / inertia, 0.0 )
		
		'' angular velocity
		S->angular_velocity = ( S->local_position_a.PerpDot( local_velocity_a * S->particle_a->mass ) + _
	                           S->local_position_b.PerpDot( local_velocity_b * S->particle_b->Mass ) ) * _
	                           S->inverse_inertia
