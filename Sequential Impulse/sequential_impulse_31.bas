...
''
''  Version 0.31, november 2016, Michael "h4tt3n" Nissen
''
...
Sub SimulationType.Demo1()
	...
	For i As Integer = 1 To num_Springs
		...
		Dim As Single inverseMass = IIf( ( S->particle_a->inverse_mass + S->particle_b->inverse_mass ) > 0.0 , _
		                                   1.0 / ( S->particle_a->inverse_mass + S->particle_b->inverse_mass ) , 0.0 )
...
Sub SimulationType.Demo4()
	...
	For S As SpringType Ptr = SpringLo To SpringHi
		...
		Dim As Single im = IIf( ( S->particle_a->inverse_mass + S->particle_b->inverse_mass ) > 0.0 , _
		                                   1.0 / ( S->particle_a->inverse_mass + S->particle_b->inverse_mass ) , 0.0 )
