...
Const As Single  gravity        = 100.0             ''  gravity
...
Sub SimulationType.Demo1()
	...
	B1->inverse_mass = 0.0'1.0 / B1->mass
...
Sub SimulationType.ComputeRestimpulse()
	...
	For J As JointType Ptr = JointLo To JointHi
		...
		'Dim As Single ia = -r_a.perpdot( distance_error.Unit() )
		'Dim As Single ib = -r_b.perpdot( distance_error.Unit() )
		
		'Dim As Single ia = -r_a.perpdot( Vec2( Sgn( distance_error.x ), Sgn( distance_error.y ) ) )
		'Dim As Single ib = -r_b.perpdot( Vec2( Sgn( distance_error.x ), Sgn( distance_error.y ) ) )
		
		Dim As Single ia = -r_a.perpdot( distance_error.Unit() )
		Dim As Single ib = -r_b.perpdot( distance_error.Unit() )
		
		J->reduced_mass = 1.0 / ( J->box_a->inverse_mass + J->box_b->inverse_mass + J->box_a->inverse_inertia * ia * ia + J->box_b->inverse_inertia * ib * ib  )
...
Sub SimulationType.ApplyCorrectiveimpulse()
	...
	For J As JointType Ptr = JointLo To JointHi
		...
		J->box_a->angular_impulse -= corrective_impulse.perpdot( -r_a ) * J->box_a->inverse_inertia' * 0.1
		J->box_b->angular_impulse += corrective_impulse.perpdot( -r_b ) * J->box_b->inverse_inertia' * 0.1
