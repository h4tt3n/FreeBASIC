...
''
''  Version 0.41, july 2017, Michael "h4tt3n" Nissen
''
...
Sub SimulationType.Demo1()
	...
	For i As Integer = 2 To num_boxes
		...
		If ( i = num_boxes ) Then
			...
			B->mass = ( 2.0 * B->radius.x ) * ( 2.0 * B->radius.y ) * density * 100
			B->inverse_mass = 1.0 / B->mass
	...
	'For i As Integer = 2 To num_boxes
	'	
	'	box(i).position += Vec2().RandomizeCircle( 256 )
	'	
	'Next
...
Sub SimulationType.Demo6()
	...
	For i As Integer = 1 To num_boxes
		...
		B->mass = 10.0'B->radius.x * B->radius.y * density
		B->inverse_mass = 1.0 / B->mass
		...
		If ( i = num_boxes Or i = 1) Then
			...
			B->mass = B->radius.x * B->radius.y * density * 200.0
...
Sub SimulationType.ComputeRestimpulse()
	...
	If ( numhinges ) Then
		...
		For J As HingeType Ptr = HingeLo To HingeHi
			...
			'Dim As Vec2 velocity_a = J->box_a->velocity + J->box_a->angular_velocity * J->r_a.perpccw
			'Dim As Vec2 velocity_b = J->box_b->velocity + J->box_b->angular_velocity * J->r_b.perpccw
			...
			Dim As Single ia = J->r_a.perpdot( distance )
			Dim As Single ib = J->r_b.perpdot( distance )
			
			Dim As Single m1 = J->box_a->inverse_mass + J->box_b->inverse_mass
			Dim As Single m2 = J->box_a->inverse_inertia * J->r_a.x * J->r_a.x + J->box_a->inverse_inertia * J->r_a.y * J->r_a.y - J->box_a->inverse_inertia * J->r_a.x * J->r_a.y - J->box_a->inverse_inertia * J->r_a.x * J->r_a.y
			Dim As Single m3 = J->box_b->inverse_inertia * J->r_b.x * J->r_b.x + J->box_b->inverse_inertia * J->r_b.y * J->r_b.y - J->box_b->inverse_inertia * J->r_b.x * J->r_b.y - J->box_b->inverse_inertia * J->r_b.x * J->r_b.y
			
			'Dim As Single m2 = J->box_a->inverse_inertia * ( J->r_a.x * J->r_a.x + J->r_a.y * J->r_a.y - J->r_a.x * J->r_a.y )
			'Dim As Single m3 = J->box_b->inverse_inertia * ( J->r_b.x * J->r_b.x + J->r_b.y * J->r_b.y - J->r_b.x * J->r_b.y )
			
			'Dim As Single m2 = J->box_a->inverse_inertia * ( J->r_a.dot( J->r_a ) - J->r_a.x * J->r_a.y )
			'Dim As Single m3 = J->box_b->inverse_inertia * ( J->r_b.dot( J->r_b ) - J->r_b.x * J->r_b.y  )
			
			J->reduced_mass = 1.0 / ( m1 + m2 + m3 )
			
			'J->reduced_mass = 1.0 / ( J->box_a->inverse_mass + J->box_b->inverse_mass + _
			'	                       J->box_a->inverse_inertia * ia * ia + J->box_b->inverse_inertia * ib * ib )
				
			'Dim As Vec2 distance_error = distance
			'Dim As Vec2 velocity_error = velocity
			
			J->rest_impulse = -spring_stiffnes * distance * inv_dt - spring_damping * velocity
...
Sub SimulationType.ApplyCorrectiveimpulse()
	...
	If ( numhinges ) Then
		...
		For J As HingeType Ptr = HingeLo To HingeHi
			...
			Dim As Vec2 impulse_a = J->box_a->impulse + J->box_a->angular_impulse * J->r_a.perpccw
			Dim As Vec2 impulse_b = J->box_b->impulse + J->box_b->angular_impulse * J->r_b.perpccw
			...
			Dim As Vec2 corrective_impulse = -impulse_error * J->reduced_mass
			
			J->box_a->impulse -= corrective_impulse * J->box_a->inverse_mass
			J->box_b->impulse += corrective_impulse * J->box_b->inverse_mass
			
			J->box_a->angular_impulse -= J->r_a.PerpDot( corrective_impulse ) * J->box_a->inverse_inertia' * 0.1
			J->box_b->angular_impulse += J->r_b.PerpDot( corrective_impulse ) * J->box_b->inverse_inertia' * 0.1
...
Sub SimulationType.ApplyWarmStart()
	...
	'	For J As HingeType Ptr = HingeLo To HingeHi
	'		
	'		Dim As Single projected_impulse = J->unit.dot( J->accumulated_impulse )
	'		
	'		If ( projected_impulse < 0.0 ) Then
	'			
	'			Dim As Vec2 warmstart_impulse = spring_warmstart * projected_impulse * J->Unit
	'			
	'			J->box_a->impulse -= warmstart_impulse * J->box_a->inverse_mass
	'			J->box_b->impulse += warmstart_impulse * J->box_b->inverse_mass
	'			
	'			J->box_a->angular_impulse += warmstart_impulse.perpdot( J->r_a ) * J->box_a->inverse_inertia
	'			J->box_b->angular_impulse -= warmstart_impulse.perpdot( J->r_b ) * J->box_b->inverse_inertia
	'			
	'		EndIf
	'		
	'		J->accumulated_impulse = Vec2( 0.0, 0.0 )
