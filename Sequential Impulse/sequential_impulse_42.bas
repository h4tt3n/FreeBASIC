...
''
''  Version 0.42, july 2017, Michael "h4tt3n" Nissen
''
...
Sub SimulationType.Demo1()
	...
	Dim As Integer num_Particles = 9
	Dim As Integer num_Springs   = 8
	Dim As Integer SpringLength  = 50
	
	Dim As Integer num_boxes  = 9
	Dim As Integer num_hinges = 8
	Dim As vec2    box_radius = vec2( 15, 5 )
	...
	For i As Integer = 2 To num_boxes
		...
		B->position.x   = 300 + (i-1) * ( box_radius.x + 10 ) * 2
		B->position.y   = 100
	...
	For i As Integer = 1 To num_hinges
		...
		J1->anchor_a = Vec2( 300 + ( box_radius.x + 10 ) + (i-1) * ( box_radius.x + 10 ) * 2, 100 ) - J1->box_a->position
		J1->anchor_b = Vec2( 300 + ( box_radius.x + 10 ) + (i-1) * ( box_radius.x + 10 ) * 2, 100 ) - J1->box_b->position
...
Sub SimulationType.ComputeRestimpulse()
	...
	If ( numhinges ) Then
		...
		For J As HingeType Ptr = HingeLo To HingeHi
			...
			'Dim As Single ia = J->Unit.perpdot( J->r_a )
			'Dim As Single ib = J->Unit.perpdot( J->r_b )
			
			''
			'J->reduced_mass = 1.0 / ( J->box_a->inverse_mass + J->box_a->inverse_inertia * ia * ia +_
			'                          J->box_b->inverse_mass + J->box_b->inverse_inertia * ib * ib )
...
Sub SimulationType.ApplyCorrectiveimpulse()
	...
	If ( numhinges ) Then
		...
		For J As HingeType Ptr = HingeLo To HingeHi
			...
			Dim As Vec2 corrective_impulse = -impulse_error * J->M * J->Unit
