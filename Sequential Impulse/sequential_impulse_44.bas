...
''
''  Version 0.44, october 2018, Michael "h4tt3n" Nissen
''
''  This version includes rigid bodies and hinges. Impulses directly influence velocity.
''
''  Controls:
''  
''  Demos                      :  F1 - F6
...
Sub SimulationType.ComputeRestimpulse()
	...
	If ( numhinges ) Then
		...
		For J As HingeType Ptr = HingeLo To HingeHi
			...
			Dim As Single rna = J->r_a.dot( distance_error )
			Dim As Single rnb = J->r_b.dot( distance_error )
			
			Dim As Single rta = J->r_a.perpdot( distance_error )
			Dim As Single rtb = J->r_b.perpdot( distance_error )
			
			Dim As Single Kn = J->box_a->inverse_mass + J->box_b->inverse_mass + _
			                   J->box_a->inverse_inertia * ( J->r_a.dot( J->r_a ) - rna * rna ) + _
			                   J->box_b->inverse_inertia * ( J->r_b.dot( J->r_b ) - rnb * rnb )
			                   
			Dim As Single Kt = J->box_a->inverse_mass + J->box_b->inverse_mass + _
			                   J->box_a->inverse_inertia * ( J->r_a.dot( J->r_a ) - rta * rta ) + _
			                   J->box_b->inverse_inertia * ( J->r_b.dot( J->r_b ) - rtb * rtb )
			
			J->reduced_mass = 1.0 / ( Kn + Kt )
			
			'''
			'Dim As Mat22 K1 = Mat22( _
			'J->box_a->inverse_mass + J->box_b->inverse_mass, 0.0f, _
			'0.0f									 	              , J->box_a->inverse_mass + J->box_b->inverse_mass )
...
Sub SimulationType.ApplyCorrectiveimpulse()
	...
	If ( numsprings ) Then
		...
		For S As SpringType Ptr = SpringLo To SpringHi
			...
			Dim As Vec2 current_impulse = S->particle_b->velocity - S->particle_a->velocity
	...
	If ( numhinges ) Then
		...
		For J As HingeType Ptr = HingeLo To HingeHi
			...
			Dim As Vec2 impulse_a = J->box_a->velocity + J->r_a.PerpDot( J->box_a->angular_velocity )
			Dim As Vec2 impulse_b = J->box_b->velocity + J->r_b.PerpDot( J->box_b->angular_velocity )
			
			Dim As Vec2 current_impulse = impulse_b - impulse_a
			
			Dim As Vec2 impulse_error = current_impulse - J->rest_impulse
			
			Dim As Vec2 corrective_impulse = -impulse_error * J->reduced_mass 'J->M
			...
			J->box_a->velocity -= corrective_impulse * J->box_a->inverse_mass
			J->box_b->velocity += corrective_impulse * J->box_b->inverse_mass
			
			J->box_a->angular_velocity -= J->r_a.PerpDot( corrective_impulse ) * J->box_a->inverse_inertia
			J->box_b->angular_velocity += J->r_b.PerpDot( corrective_impulse ) * J->box_b->inverse_inertia
