...
Const As Integer max_particles   = 512             ''  particles
Const As Integer max_springs     = 512             ''  springs
Const As Integer max_boxes       =  128             ''  boxes
Const As Integer max_RevoluteJoints =  128             ''  joints
Const As Integer max_Worldjoints    =  128             ''  joints
Const As Integer max_AngularJoints  =  128      ''  
Const As Integer max_DistanceJoints =  128     ''  
Const As Integer max_LineJoints     =  128     ''  
...
Type SimulationType
	...
	As RevoluteJointType      joint      ( 1 To max_RevoluteJoints )
...
Sub SimulationType.Demo1()
	...
	For i As Integer = 1 To num_boxes
		...
		If ( i = num_boxes ) Then
			...
			B->mass = ( 2.0 * B->radius.x ) * ( 2.0 * B->radius.y ) * density * 10
			B->inverse_mass = 1.0 / B->mass
	...
	For i As Integer = 1 To num_joints
		...
		'Dim As DistanceJointType Ptr J1 = CreateDistanceJoint()
	...
	L1->box_a = @box(1)
	L1->box_b = @box(num_boxes)
	
	L1->anchor_a = Vec2( 0.0, 0.0 )
	L1->anchor_b = Vec2( 80.0,   0.0 )
	
	L1->unit = Vec2( 0.0, 1.0 )
	
	Dim As LineJointType Ptr L2 = CreateLineJoint()
	
	L2->box_a = @box(1)
	L2->box_b = @box(num_boxes)
	
	L2->anchor_a = Vec2( 0.0,  0.0 )
	L2->anchor_b = Vec2( -80.0, 0.0 )
	
	L2->unit = Vec2( 1.0, 0.0 )
...
Sub SimulationType.ComputeRestimpulse()
	...
	If ( numAngularJoints ) Then
		...
		For S As AngularJointType Ptr = AngularJointLo To AngularJointHi
			...
			Dim As Single velocity = S->box_b->angular_velocity - S->box_a->angular_velocity
			
			'' error
			Dim As Single angle_error = angle - S->rest_angle
			Dim As Single velocity_error = velocity - S->rest_velocity
			
			'' Correction
			S->rest_impulse = 0.0
			S->rest_impulse -= IIf( spring_stiffnes > 0.0 , spring_stiffnes * angle_error * inv_dt , 0.0 )
			S->rest_impulse -= IIF( spring_damping > 0.0 , spring_damping * velocity_error, 0.0 )
	...
	If ( numDistanceJoints ) Then
		...
		For J As DistanceJointType Ptr = DistanceJointLo To DistanceJointHi
			...
			Dim As Single K2 = J->box_a->inverse_inertia * J->unit.dot( J->r_a.perpdot( J->r_a.perpdot( J->unit ) ) )
			Dim As Single K3 = J->box_b->inverse_inertia * J->unit.dot( J->r_b.perpdot( J->r_b.perpdot( J->unit ) ) )
	...
	If ( numLineJoints ) Then
		...
		For J As LineJointType Ptr = LineJointLo To LineJointHi
			...
			'' State
			J->n = J->box_a->angle_vector.Rotateccw( J->unit )
			
			Dim As Vec2 t = J->n.perpccw()
			
			Dim As vec2 r_a = J->box_a->angle_vector.Rotateccw( J->anchor_a )

			J->r_b = J->box_b->angle_vector.Rotateccw( J->anchor_b )
			
			J->r_a = r_a + J->n.dot( ( J->box_b->position + J->r_b ) - ( J->box_a->position + r_a ) ) * J->n
			...
			'Dim As Single K2 = J->box_a->inverse_inertia * ( J->r_a.perpdot( J->r_a.perpdot( t ) ) ).dot( t )
			'Dim As Single K3 = J->box_b->inverse_inertia * ( J->r_b.perpdot( J->r_b.perpdot( t ) ) ).dot( t )
			
			Dim As Single K2 = J->box_a->inverse_inertia * t.dot( J->r_a.perpdot( J->r_a.perpdot( t ) ) )
			Dim As Single K3 = J->box_b->inverse_inertia * t.dot( J->r_b.perpdot( J->r_b.perpdot( t ) ) )
...
Sub SimulationType.ApplyCorrectiveimpulse()
	...
	If ( numWorldjoints ) Then
		...
		For J As WorldRevoluteJointType Ptr = WorldjointLo To WorldjointHi
			...
			Dim As Vec2 current_impulse = J->box->impulse + J->r.PerpDot( J->box->angular_impulse )
	...
	If ( numAngularJoints ) Then
		...
		For A As AngularJointType Ptr = AngularJointLo To AngularJointHi
			...
			Dim As Single impulse_error = IIf( Not A->rest_impulse = 0.0 , current_impulse - A->rest_impulse , 0.0 )
...
Sub SimulationType.ApplyWarmStart()
	...
	If ( numLineJoints ) Then
		...
		For L As LineJointType Ptr = LineJointLo To LineJointHi
			
			Dim As Vec2 t = L->n.perpccw()
			
			Dim As Single projected_impulse = t.dot( L->accumulated_impulse )
			
			If ( projected_impulse < 0.0 ) Then
				
				Dim As Vec2 warmstart_impulse = spring_warmstart * projected_impulse * t
				
				L->box_a->impulse -= warmstart_impulse * L->box_a->inverse_mass
				L->box_b->impulse += warmstart_impulse * L->box_b->inverse_mass
				
				L->box_a->angular_impulse -= L->r_a.PerpDot( warmstart_impulse ) * L->box_a->inverse_inertia
				L->box_b->angular_impulse += L->r_b.PerpDot( warmstart_impulse ) * L->box_b->inverse_inertia
...
Sub SimulationType.UpdateScreen()
	...
	If ( numLineJoints ) Then
		...
		For J As LineJointType Ptr = LineJointLo To LineJointHi
			...
			Circle( r_a.x, r_a.y ), 2.0, RGB( 255, 255, 0 ),,, 1, f
			Circle( r_b.x, r_b.y ), 2.0, RGB( 255, 255, 0 ),,, 1, f

			Line( r_a.x, r_a.y )-( r_b.x, r_b.y ), RGB( 255, 255, 0 )
...
Sub SimulationType.Clearjoints()
	...
	For i As Integer = 1 To max_RevoluteJoints
...
Function SimulationType.Createjoint() As RevoluteJointType Ptr
	...
	If ( numjoints < max_RevoluteJoints - 1 ) Then
