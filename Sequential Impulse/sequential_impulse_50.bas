...
''
''  Version 0.50, August 2017, Michael "h4tt3n" Nissen
''
...
Sub SimulationType.RunSimulation()
	...
	'Dim As Single delta_angle1 = -0.0001 * 2 * PI
...
Sub SimulationType.Demo2()
	...
	For i As integer = 1 To num_Particles
		...
		P->position = 0.5 * Vec2( screen_wid, screen_hgt * 0.125 ) + Vec2( Cos(Angle), Sin(Angle) ) * SpringLength
...
Sub AngularSpringType.ComputeCorrectiveImpulse()
	...
	S_a->particle_a->Impulse -= ( S_a->Particle_a->Position - S_a->Position ).PerpDot( new_Angular_Impulse_a )
	S_a->particle_b->Impulse -= ( S_a->Particle_b->Position - S_a->Position ).PerpDot( new_Angular_Impulse_a )
	
	S_b->particle_a->Impulse += ( S_b->Particle_a->Position - S_b->Position ).PerpDot( new_Angular_Impulse_b )
	S_b->particle_b->Impulse += ( S_b->Particle_b->Position - S_b->Position ).PerpDot( new_Angular_Impulse_b )
