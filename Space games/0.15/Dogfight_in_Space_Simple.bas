...
For i = 1 To Ubound(Debris)
    With Debris(i) 
        ...
        If Int(Rnd*2) = 0 Then
            .Orbit_Radius = Planet(0).Radius + 700 + (Rnd*(500^(1/3)))^3
        Else
            .Orbit_Radius = Planet(0).Radius + 700 - (Rnd*(500^(1/3)))^3
        End If
...
For i = 0 To Ubound(Emerald) 
    With Emerald(i) 
        ...
        .Angle = Rnd*(toxpi)
        If Int(Rnd*2) = 0 Then
            .Orbit_Radius = Planet(0).Radius + 1000 + (Rnd*(500^(1/3)))^3
        Else
            .Orbit_Radius = Planet(0).Radius + 1000 - (Rnd*(500^(1/3)))^3
        End If
...
For i = Lbound(Ship) To Ubound(Ship)
    With Ship(i)
        .state = 1
        .Orbit_Radius = Planet(0).Radius+500
        .X = Planet(0).X+.Orbit_Radius*Sin(.Angle)
        .Y = Planet(0).Y+.Orbit_Radius*Cos(.Angle)
        .Fuel_Mass = 500
        .Mass = 500 + .Fuel_Mass
        ...
        For i2 = 0 to Ubound(.Burst)
            With .Burst(i2)
                .Speed = screen_update*(10+Rnd*100)
                .State = 0
                .Time = 60
                .R = 255
                .G = 255
        ...
        If Planet(i).State = 1 Then
            For i2 = i+1 To Ubound(Planet)
                If Planet(i2).State = 1 Then
                    ...
                    If Distance < MinDist then
                        Distance = MinDist
                    End If
                    Gforce = Spec_Grav*((Planet(i).Mass*Planet(i2).Mass)/(Distance^2))
                    G_i2mass = Gforce/Planet(i2).Mass
                    If i <> 0 Then
                        G_imass = Gforce/Planet(i).Mass
            ...
            For i2 = 0 To Ubound(Ship)
                For i3 = 0 to Ubound(Ship(i2).Mine)
                    If Ship(i2).Mine(i3).State = 1 Then
                        ...
                        End If
                        Gforce = Spec_Grav*((Planet(i).Mass*Ship(i2).Mine(i3).Mass)/(Distance^2))
                        G_i2mass = Gforce/Ship(i2).Mine(i3).Mass
                        If i <> 0 Then
                            G_imass = Gforce/Planet(i).Mass
                            Planet(i).Xvec -= (Xdist/Distance)*G_imass
                            Planet(i).Yvec -= (Ydist/Distance)*G_imass
                ...
                For i3 = 0 to Ubound(Ship(i2).Rocket)
                    If Ship(i2).Rocket(i3).State = 1 Then
                        ...
                        End If
                        Gforce = Spec_Grav*((Planet(i).Mass*Ship(i2).Rocket(i3).Mass)/(Distance^2))
                        G_i2mass = Gforce/Ship(i2).Rocket(i3).Mass
                        If i <> 0 Then
                            G_imass = Gforce/Planet(i).Mass
                            Planet(i).Xvec -= (Xdist/Distance)*G_imass
                            Planet(i).Yvec -= (Ydist/Distance)*G_imass
                ...
                If Ship(i2).State = 1 Then
                    ...
                    If Distance < MinDist then
                        ...
                        ship(i2).Yvec *= 0.9
                    End If
                    Gforce = Spec_Grav*((Planet(i).Mass*Ship(i2).Mass)/(Distance^2))
                    G_i2mass = Gforce/Ship(i2).Mass
                    If i <> 0 Then
                        G_imass = Gforce/Planet(i).Mass
                        Planet(i).Xvec -= (Xdist/Distance)*G_imass
                        Planet(i).Yvec -= (Ydist/Distance)*G_imass
            ...
            For i2 = 0 To Ubound(Debris)
                If Debris(i2).State = 1 Then
                    ...
                    End If
                    Gforce = Spec_Grav*((Planet(i).Mass*Debris(i2).Mass)/(Distance^2))
                    G_i2mass = Gforce/Debris(i2).Mass
                    If i <> 0 Then
                        G_imass = Gforce/Planet(i).Mass
                        Planet(i).Xvec -= (Xdist/Distance)*G_imass
                        Planet(i).Yvec -= (Ydist/Distance)*G_imass
            ...
            For i2 = 0 To Ubound(Emerald)
                If Emerald(i2).State = 1 Then
                    ...
                    If Distance < MinDist then
                        ...
                        Next
                    End If
                    Gforce = Spec_Grav*((Planet(i).Mass*Emerald(i2).Mass)/(Distance^2))
                    G_i2mass = Gforce/Emerald(i2).Mass
                    If i <> 0 Then
                        G_imass = Gforce/Planet(i).Mass
                        Planet(i).Xvec -= (Xdist/Distance)*G_imass
                        Planet(i).Yvec -= (Ydist/Distance)*G_imass
                ...
                For i2 = 0 to Ubound(Debris)
                    If Debris(i2).State = 1 Then
                        ...
                        If Distance < Ship(i).Shield_Radius+Debris(i2).Radius Then
                            Shield_Strength = 100+(0.1*Debris(i2).Mass)
                ...
                For i2 = 0 to Ubound(Emerald)
                    If Emerald(i2).State = 1 Then
                        ...
                        If Distance < Ship(i).Shield_Radius+Emerald(i2).Radius Then
                            Shield_Strength = 100+(0.1*Emerald(i2).Mass)
                            Ship(i).Xvec += Shield_Strength*(Xdist/Distance)/Ship(i).Mass
                            Ship(i).Yvec += Shield_Strength*(Ydist/Distance)/Ship(i).Mass
                ...
                For i2 = Lbound(Ship) to Ubound(Ship)
                    If i2 <> i Then
                        If Ship(i2).State = 1 Then
                            ...
                            If Distance <= MinDist Then
                                Shield_Strength = 100+(0.1*Ship(i2).Mass)
                                Ship(i).Xvec += Shield_Strength*(Xdist/Distance)/Ship(i).Mass
                                Ship(i).Yvec += Shield_Strength*(Ydist/Distance)/Ship(i).Mass
                        ...
                        If Ship(i2).Projectile(i3).State = 1 Then
                            If Ship(i2).Projectile(i3).timeleft < Ship(i2).Projectile(i3).time-6 Then
                                ...
                                If Distance <= Ship(i).Shield_Radius Then
                                    Shield_Strength = 100+(0.1*Ship(i2).Projectile(i3).Mass)
                                    Ship(i).Xvec += Shield_Strength*(Xdist/Distance)/Ship(i).Mass
                                    Ship(i).Yvec += Shield_Strength*(Ydist/Distance)/Ship(i).Mass
            ...
            If .State = 1 Then
                ...
                If multikey (.fwd) then
                    If .Fuel_Mass >= 0.02 then
                        .Xvec += Screen_update*1800*(Cos(.Direction*Radtodeg)/.Mass)
                        .Yvec += Screen_update*1800*(Sin(-.Direction*Radtodeg)/.Mass)
                        .Fuel_Mass -= 0.02
                        .Mass -= 0.02
                        For i2 = 0 to Ubound(.Burst)
        ...
        If .State = 1 Then
            ...
            For i = 0 To Ubound(Planet)
                If Planet(i).State = 1 Then
                    ...
                    Distance = Sqr((.X-Xpos)^2+(.Y-Ypos)^2)
