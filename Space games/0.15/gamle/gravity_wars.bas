...
Do 
    ...
    For i = 0 To Ubound(Planet)
        ...
        For i2 = 0 To Ubound(Emerald)  '' from 0 to # of emerald asteroids
            If Planet(i).State = 1 and Emerald(i2).State = 1 Then
                Distance = sqr(((Planet(i).Y-Emerald(i2).Y)^2) + ((Planet(i).X-Emerald(i2).X)^2))
                MinDist = Planet(i).Radius+Emerald(i2).Radius
                    If Distance < MinDist then
                        Distance = MinDist
                    End If
                Gforce = Gravity*((Planet(i).Mass*Emerald(i2).Mass)/(Distance^2))
                Emerald(i2).Xvec -= ((Emerald(i2).X-Planet(i).X)/Distance)*(Gforce/Emerald(i2).Mass)
                Emerald(i2).Yvec -= ((Emerald(i2).Y-Planet(i).Y)/Distance)*(Gforce/Emerald(i2).Mass)
                Planet(i).Xvec -= ((Planet(i).X-Emerald(i2).X)/Distance)*(Gforce/Planet(i).Mass)
                Planet(i).Yvec -= ((Planet(i).Y-Emerald(i2).Y)/Distance)*(Gforce/Planet(i).Mass)
            End If
        Next
        For i2 = 0 To Ubound(Comet)  '' from 0 to # of comets
            If Planet(i).State = 1 and Comet(i2).State = 1 Then
                Distance = sqr(((Planet(i).Y-Comet(i2).Y)^2) + ((Planet(i).X-Comet(i2).X)^2))
                MinDist = Planet(i).Radius+Comet(i2).Radius
                    If Distance < MinDist then
                        Distance = MinDist
                    End If
                Gforce = Gravity*((Planet(i).Mass*Comet(i2).Mass)/(Distance^2))
                Comet(i2).Xvec -= ((Comet(i2).X-Planet(i).X)/Distance)*(Gforce/Comet(i2).Mass)
                Comet(i2).Yvec -= ((Comet(i2).Y-Planet(i).Y)/Distance)*(Gforce/Comet(i2).Mass)
                Planet(i).Xvec -= ((Planet(i).X-Comet(i2).X)/Distance)*(Gforce/Planet(i).Mass)
                Planet(i).Yvec -= ((Planet(i).Y-Comet(i2).Y)/Distance)*(Gforce/Planet(i).Mass)
            End If
        Next
        For i2 = 0 To Ubound(Comet_Debris)  '' from 0 to # of comet debris
            If Planet(i).State = 1 and Comet_Debris(i2).State = 1 Then
                Distance = sqr(((Planet(i).Y-Comet_Debris(i2).Y)^2) + ((Planet(i).X-Comet_Debris(i2).X)^2))
                MinDist = Planet(i).Radius+Comet_Debris(i2).Radius
                    If Distance < MinDist then
                        Distance = MinDist
                    End If
                Gforce = Gravity*((Planet(i).Mass*Comet_Debris(i2).Mass)/(Distance^2))
                Comet_Debris(i2).Xvec -= ((Comet_Debris(i2).X-Planet(i).X)/Distance)*(Gforce/Comet_Debris(i2).Mass)
                Comet_Debris(i2).Yvec -= ((Comet_Debris(i2).Y-Planet(i).Y)/Distance)*(Gforce/Comet_Debris(i2).Mass)
                Planet(i).Xvec -= ((Planet(i).X-Comet_Debris(i2).X)/Distance)*(Gforce/Planet(i).Mass)
                Planet(i).Yvec -= ((Planet(i).Y-Comet_Debris(i2).Y)/Distance)*(Gforce/Planet(i).Mass)
            End If
        Next
    Next
    For i = 0 to Ubound(Station)
        ...
        For i2 = 0 To Ubound(Emerald)  '' from 0 to # of emerald asteroids
            If Station(i).State = 1 and Emerald(i2).State = 1 Then
                Distance = sqr(((Station(i).Y-Emerald(i2).Y)^2) + ((Station(i).X-Emerald(i2).X)^2))
                Gforce = Gravity*((Station(i).Mass*Emerald(i2).Mass)/(Distance^2))
                MinDist = Station(i).Radius+Emerald(i2).Radius
                    If Distance < MinDist then
                        Distance = MinDist
                    End If
                Emerald(i2).Xvec -= ((Emerald(i2).X-Station(i).X)/Distance)*(Gforce/Emerald(i2).Mass)
                Emerald(i2).Yvec -= ((Emerald(i2).Y-Station(i).Y)/Distance)*(Gforce/Emerald(i2).Mass)
                Station(i).Xvec -= ((Station(i).X-Emerald(i2).X)/Distance)*(Gforce/Station(i).Mass)
                Station(i).Yvec -= ((Station(i).Y-Emerald(i2).Y)/Distance)*(Gforce/Station(i).Mass)
            End If
        Next
        For i2 = 0 To Ubound(Comet)  '' from 0 to # of comets
            If Station(i).State = 1 and Comet(i2).State = 1 Then
                Distance = sqr(((Station(i).Y-Comet(i2).Y)^2) + ((Station(i).X-Comet(i2).X)^2))
                MinDist = Station(i).Radius+Comet(i2).Radius
                    If Distance < MinDist then
                        Distance = MinDist
                    End If
                Gforce = Gravity*((Station(i).Mass*Comet(i2).Mass)/(Distance^2))
                Comet(i2).Xvec -= ((Comet(i2).X-Station(i).X)/Distance)*(Gforce/Comet(i2).Mass)
                Comet(i2).Yvec -= ((Comet(i2).Y-Station(i).Y)/Distance)*(Gforce/Comet(i2).Mass)
                Station(i).Xvec -= ((Station(i).X-Comet(i2).X)/Distance)*(Gforce/Station(i).Mass)
                Station(i).Yvec -= ((Station(i).Y-Comet(i2).Y)/Distance)*(Gforce/Station(i).Mass)
            End If
        Next
        For i2 = 0 To Ubound(Comet_Debris)  '' from 0 to # of comet debris
            If Station(i).State = 1 and Comet_Debris(i2).State = 1 Then
                Distance = sqr(((Station(i).Y-Comet_Debris(i2).Y)^2) + ((Station(i).X-Comet_Debris(i2).X)^2))
                MinDist = Station(i).Radius+Comet_Debris(i2).Radius
                    If Distance < MinDist then
                        Distance = MinDist
                    End If
                Gforce = Gravity*((Station(i).Mass*Comet_Debris(i2).Mass)/(Distance^2))
                Comet_Debris(i2).Xvec -= ((Comet_Debris(i2).X-Station(i).X)/Distance)*(Gforce/Comet_Debris(i2).Mass)
                Comet_Debris(i2).Yvec -= ((Comet_Debris(i2).Y-Station(i).Y)/Distance)*(Gforce/Comet_Debris(i2).Mass)
                Station(i).Xvec -= ((Station(i).X-Comet_Debris(i2).X)/Distance)*(Gforce/Station(i).Mass)
                Station(i).Yvec -= ((Station(i).Y-Comet_Debris(i2).Y)/Distance)*(Gforce/Station(i).Mass)
            End If
        Next
    Next
             ...
             For i2 = 0 to Ubound(Asteroid)
                If Asteroid(i2).State = 1 Then
                    ...
                    If Distance < Ship(i).Shield_Minsize+2+sqr(Ship(i).Shield_Strength/pi)+Asteroid(i2).Radius Then
                        Asteroid(i2).Xvec += (300*((Asteroid(i2).X-Ship(i).X)/Distance))/Asteroid(i2).Mass
                        Asteroid(i2).Yvec += (300*((Asteroid(i2).Y-Ship(i).Y)/Distance))/Asteroid(i2).Mass
                        Ship(i).Xvec += (300*((Ship(i).X-Asteroid(i2).X)/Distance))/Ship(i).Mass
                        Ship(i).Yvec += (300*((Ship(i).Y-Asteroid(i2).Y)/Distance))/Ship(i).Mass
                    End If
                End If
            Next
            For i2 = 0 to Ubound(Comet_Debris)
    ...
    ''  print fps
    Color Rgb(16, 96, 16)
    Print using "Prgm: ### fps"; FPS
    Print using "Scrn: ### fps"; Screen_Rate
