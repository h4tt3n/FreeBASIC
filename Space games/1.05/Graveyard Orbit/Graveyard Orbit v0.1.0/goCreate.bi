...
Function Game.CreatePlanet( ByVal _mass As Single, ByVal _density As Single ) As Planet Ptr
	
	Dim As Planet P = Planet( _mass, _density )
...
Function Game.CreateAsteroidBelt( ByVal _Parent        As LinearState Ptr, _
	...
	SP->LinearStates_.Reserve( _Asteroids + 1 ) 
	
	For i As Integer = 0 To _Asteroids - 2
		
		Dim As Single mass = 1E1 + Rnd() * 9E1
		
		Dim As Single density = 8E-3
		
		Dim As Planet Ptr P = CreatePlanet( mass, density )
		
		P->SetColour( RGB( 136, 128, 120 ) )
...
Function Game.CreateStrongGirder( ByVal _points As Vec2, _
	...
	If ( _type = S_TRUSS Or _type = X_TRUSS ) Then
		...
		For y as integer = 0 to _points.y - 2
			For x as integer = 1 to ( _points.x * 0.5 ) - 2
