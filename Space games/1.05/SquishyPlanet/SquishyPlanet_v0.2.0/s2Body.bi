...
Sub Body.ApplyImpulseConcentration( ByVal c As Single )
	...
	For I as Particle Ptr Ptr = Particles_.Front To Particles_.Back
		...
		Dim As Vec2 LocalImpulse = P->Impulse * P->Mass * InverseMass_ 
		
		''  Compute global force from local forces
		Impulse_ += LocalImpulse * c
		
		''  Compute global torque from local forces
		Dim As Vec2 LocalPosition = Position_ - P->Position
		
		'Dim As Single LocalInertia = LocalPosition.LengthSquared() * P->Mass
		
		Dim As Single LocalAngularImpulse = LocalPosition.PerpDot( P->Impulse ) * InverseInertia_ * P->Mass
		
		AngularImpulse_ -= LocalAngularImpulse * c
		
		'' reset local 
		P->Impulse( Vec2( 0.0, 0.0 ) )
		'P->Impulse( P->Impulse * ( 1.0 - c ) )
		'P->AddImpulse( -LocalImpulse * ( 1.0 - c ) )
		'P->AddImpulse( LocalPosition.PerpDot( LocalAngularImpulse  ) * InverseInertia_ * P->Mass * ( 1.0 - c ) )
...
Sub Body.ApplyImpulseDispersion()
	...
	For I as Particle Ptr Ptr = Particles_.Front To Particles_.Back
		...
		''  Compute local linear impulse from global linear impulse
		P->addImpulse( Impulse_ ) 
		
		''  Compute local linear impulse from global angular impulse
		Dim As Vec2 LocalPosition = Position_ - P->Position
