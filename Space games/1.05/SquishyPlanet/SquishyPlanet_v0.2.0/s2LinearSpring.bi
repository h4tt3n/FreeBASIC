...
''
''   LineSegment --> LinearSpring / FixedSpring
''
''******************************************************************************
...
Sub LinearSpring.ApplyCorrectiveImpulse()
	...
	Dim As Single impulse_error = LengthUnit_.Dot( delta_impulse ) - RestImpulse_
	
	Dim As Vec2 corrective_impulse = -Corrective_ * impulse_error * LengthUnit_ * ReducedMass_
...
Sub LinearSpring.ApplyWarmStart()
	...
	Dim As Single projected_impulse = LengthUnit_.Dot( AccumulatedImpulse_ )
	
	If ( projected_impulse < 0.0 ) Then
		
		Dim As Vec2 warmstart_impulse = Warmstart_ * projected_impulse * LengthUnit_
...
Sub LinearSpring.ComputeRestImpulse()
	...
	Dim As Single velocity_error = LengthUnit_.Dot( delta_velocity )
...
Sub LinearSpring.computeReducedMass()
	...
	ReducedMass_ = 1.0 / ( ParticleA_->InverseMass + ParticleB_->InverseMass )
	
End Sub


'' Get
Function LinearSpring.Corrective() As Single
...
'' Set
Sub LinearSpring.Corrective ( ByVal c As Single )
