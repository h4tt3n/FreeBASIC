...
''
Dim As LinearState Ptr SB1 = Thisworld.CreateLinearState( 1E6, Vec2( 0.0, 0.0 ) )
Dim As LinearState Ptr SB2 = Thisworld.CreateLinearState( 1E6, Vec2( 0.0, 0.0 ) )
Dim As LinearState Ptr SB3 = Thisworld.CreateLinearState( 2E6, Vec2( 0.0, 0.0 ) )
Dim As LinearState Ptr SB4 = Thisworld.CreateLinearState( 4E6, Vec2( 0.0, 0.0 ) )

Dim As SoftBody Ptr Ship = Thisworld.CreateSoftBody( 3, 64.0, Vec2( -1700.0, 8000.0 ) )


Dim As SoftBody Ptr blob = Thisworld.CreateSoftBody( 32, 2048.0, Vec2( 0.0, 0.0 ) )

'Dim As LinearState Ptr P = *blob->LinearStates_[7]
'blob->addAngularImpulse( 0.1 )
'P->addImpulse( Vec2( 1000.0, 0.0 ) )
...
Ship->LinearStates_.Resize( 1000 )

'Ship->AddObject( N1 )
'Ship->RemoveObject( 3 )  ' doesn't work
'Ship->LinearStates_.pop_back()
...
Do
	...
	For S As SoftBody Ptr = ThisWorld.SoftBodys_.p_front To ThisWorld.SoftBodys_.p_back
		...
		If S->GetFlag( IS_VISIBLE ) Then
			...
			'Dim As LinearState Ptr P = *S->LinearStates_.p_front
	...
	For P As LinearState Ptr = ThisWorld.LinearStates_.p_front To ThisWorld.LinearStates_.p_back
		...
		If P->GetFlag( IS_VISIBLE ) Then
			...
			Circle( P->Position.x, P->Position.y ), 32.0 , RGB( 128, 128, 128 ),,,1,f
