...
''
''   Description:
''   This class takes care of all the hairy orbital math and physics. THis is
''   by far the most extensive and complicated class. DO NOT fiddle around here
''   unless you really know what you are doing.
''
''   LinearState -> AngularState -> LinearLink -> KeplerOrbit
''
''   To-do list:
''
...
'' Kepler orbit class
Type KeplerOrbit Extends LinearLink
...
Constructor KeplerOrbit( ByVal A As LinearState Ptr, _
                         ByVal B As LinearState Ptr )
...
Function KeplerOrbit.computeFlightPathAngle( ByVal _eccentricity As Single, _
	...
	'FlightPathAngle_ = UnitToAngle( Vec2( 1.0 + eccentricity_, eccentricity_ ).AngularState( trueAnomalyVector_ ) )
...
Sub KeplerOrbit.ComputeFlightPathVector()
	...
	'FlightPathVector_ = ( Vec2( 1.0 + eccentricity_, eccentricity_ ) * TrueAnomalyVector_ ).AngularState( LengthVector_.Perp() )
...
Sub KeplerOrbit.ComputeHillSpheres()
	...
	Dim As Single HillSphere = semiMajorAxis_ * ( 1.0 - eccentricity_ ) * GetInverseMass
	
	HillSphereA_ = HillSphere * ParticleA_->GetMass
	HillSphereB_ = HillSphere * ParticleB_->GetMass
...
Sub KeplerOrbit.ComputeKeplerFromState()
	...
	'' Anomalies
	'ComputeTrueFromState()     '' not needed for drawing orbit
	'ComputeEccentricFromTrue() '' not needed for drawing orbit
...
Sub KeplerOrbit.ComputeStateFromKepler()
	...
	'PeriapsisVector_ = Vec2( Cos( Periapsis_ ), Sin( Periapsis_ ) )
	...
	Dim As Single RadiusAngle = TrueAnomaly_ + Periapsis_ '' change to vector format

	'Dim As Vec2 _RadiusUnitVector = AngleToUnit( RadiusAngle )
	Dim As Vec2 _RadiusUnitVector = TrueAnomalyVector_.RotateCCW( PeriapsisVector_ )
	
	LengthVector_ = _RadiusUnitVector * Length_

	ComputeVelocity()
	
	Dim As Vec2 VelocityUnitVector_ = AngleToUnit( RadiusAngle - FlightPathAngle_ ).PerpCCW() '' change to vector format
	'Dim As Vec2 VelocityUnitVector_ = FlightPathVector_.AngularState( -_RadiusUnitVector ).Perp()
...
Sub KeplerOrbit.ComputeTrueFromState()
	...
	TrueAnomalyVector_ = PeriApsisVector_.RotateCCW( AngleVector_ )
...
Sub KeplerOrbit.computeVelocityVector()
	...
	VelocityVector_ = ParticleA_->GetVelocity - ParticleB_->GetVelocity
...
Sub KeplerOrbit.DrawOrbit()
	...
	Dim As Vec2 SemiAxesA = Vec2( semiMajorAxis_, semiMinorAxis_ ) * ParticleA_->GetMass * GetInverseMass
	Dim As Vec2 SemiAxesB = Vec2( semiMajorAxis_, semiMinorAxis_ ) * ParticleB_->GetMass * GetInverseMass
	...
	''	Draw focal points
	Circle( EmptyFocusA.x, EmptyFocusA.y ), 0.03 * SemiAxesA.x,    RGB( 32, 96, 32 ),,,1
	Circle( EmptyFocusB.x, EmptyFocusB.y ), 0.03 * SemiAxesB.x,    RGB( 32, 96, 32 ),,,1
	Circle( Position_.x, Position_.y )    , 0.03 * semiMajorAxis_, RGB( 32, 96, 32 ),,,1
	
	''	Draw orbits
	FastEllipse( OrbitCenterA, SemiAxesA, PeriApsisVector_, RGB( 64, 192, 64 ) )
	FastEllipse( OrbitCenterB, SemiAxesB, PeriApsisVector_, RGB( 64, 192, 64 ) )
...
Sub KeplerOrbit.ResetVariables()
	...
	''	Reset everything except LinearState pointers
...
Sub KeplerOrbit.ApplyOrbit( ByVal _Position       As Vec2, _
	...
	Dim As Vec2 PositionVectorA =  Position_ - LengthVector_ * ParticleB_->GetMass * GetInverseMass
	Dim As Vec2 PositionVectorB =  Position_ + LengthVector_ * ParticleA_->GetMass * GetInverseMass

	Dim As Vec2 VelocityVectorA = -VelocityVector_ * ParticleB_->GetMass * GetInverseMass
	Dim As Vec2 VelocityVectorB =  VelocityVector_ * ParticleA_->GetMass * GetInverseMass

	''	Apply state vectors
	ParticleA_->SetPosition( PositionVectorA )
	ParticleB_->SetPosition( PositionVectorB )
